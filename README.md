...
- **Optional folder search cache** (off by default; **Embeddings + FTS** when enabled) builds beside your documents in `writeragent_embeddings/` (Writer `.odt`, Calc `.ods`, Impress `.odp`, Draw `.odg`): unified **`corpus.db`** (FTS5 + sqlite-vec). [Multi-document plan](docs/multi-document-dev-plan.md) · [Embeddings & folder FTS](docs/embeddings.md). **Venv packages:** `sentence-transformers numpy sqlite-vec langgraph langchain-core langchain-text-splitters envwrap odfpy`.
