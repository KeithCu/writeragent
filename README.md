...
- **Visualization**: **Viz Helpers** — `quick_plot`, `correlation_heatmap`, `time_series_plot`. Charts insert on the sheet; `=PY()` / `matplotlib` plots work the same way. **Packages:** `matplotlib`, `seaborn`. See [Visualization](docs/enabling_numpy_in_libreoffice.md#visualization).
...
- **Unit conversion (Pint)**: **Units Helpers** — `convert_quantity`, `parse_quantity`, `format_quantity`, `check_dimensionality`. On Calc, `convert_quantity` / `parse_quantity` write a **single formatted cell** at the selection anchor by default (e.g. `36 km/h`); pass `output_style: "detailed"` for a key-value grid. Goes beyond Calc's built-in `CONVERT()` for compound units like `m/s` → `km/h`. **Packages:** `pint`. See [Data Engineering / Units](docs/enabling_numpy_in_libreoffice.md#data-engineering-units).
...
- **Run Python Script**: **Tools → Run Python Script…** — built-in sections **Analysis**, **Viz**, **Math**, **Units**, **Quant**, **Optimize**, **Vision** (set **Data** range where applicable, edit `params` if needed, run). Configure venv in **Settings → Python**.
