...
- **=PYTHON() Function**: Execute Python code directly from cells using your configured venv. Use `result = ...` to return a value; pass a range as the second argument (e.g. `=PYTHON("result = sum(data[0])", A1:A10)`) to inject values as `data`.
...
- **New Calc Formula**: Use `=PYTHON("result = 3 ** 8")` or pass a range: `=PYTHON("result = sum(data[0])", A1:A10)`.
