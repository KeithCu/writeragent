...
- **Run Python Script**: **Tools → Run Python Script…** — built-in sections **Analysis**, **Viz**, **Math**, **Quant**, **Optimize**, **Vision** (set **Data** range, edit `params` if needed, run). Also available via `run_venv_python_script`, `=PY()` / `=PYTHON()`. Configure venv in **Settings → Python**.
