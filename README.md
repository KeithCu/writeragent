...
- **Specialized sub-agent (delegation)** — The main chat keeps a **small default** Writer tool list. For deep work in one area, the **main** model calls **`delegate_to_specialized_writer_toolset`** with a **`domain`** (e.g. `tables`, `styles`, `layout`, `fields`, `indexes`, …) and a **`task`** string. That starts a **nested** LLM run that only sees tools for that domain plus **`specialized_workflow_finished`**. When the subtask is done, the **nested** model must call **`specialized_workflow_finished`** with a **`summary`** so control returns to the main conversation and its default tools. Same sidebar session; different tool surface for that sub-run only.
- **Swapping tools per send or mode** — Same *pairing of ideas* (**enter specialized** → **work** → **explicit return**) but **without** a nested model: each HTTP request could attach only the domain tools (and a “done” tool) while the **same** thread keeps full history—e.g. names mirroring **`delegate_to_specialized_writer_toolset`** / **`specialized_workflow_finished`** even if implemented as a fresh **`tools`** array per turn instead of an inner agent. **Caveat**: the model still sees the whole transcript, so it may mix up which tools were valid when; clear tool text helps. **Web search** only rhymes as “branch at send”; it uses a **dedicated research flow**, not this swap.
