...
- **Unit conversion (Pint)**: **Units Helpers** — `convert_quantity`, `parse_quantity`, `format_quantity`, `check_dimensionality`. Results insert as formatted text at the selection (e.g. `10 m/s` → `36 km/h`). **Packages:** `pip install pint`. See [Data Engineering / Units](docs/enabling_numpy_in_libreoffice.md#data-engineering-units).
- **Real-time Grammar Checker**: An asynchronous proofreader with a **sentence cache** and **Unicode-aware splitting**. Includes **Token-aware Overlap Repair** to fix "LLM slop" and ensure surgical replacements. Persistent storage of good/bad sentences with document. Enable in **Settings → Doc → Enable AI grammar checker (Writer)** (off by default); underlines appear shortly after you pause typing. Optional **sentence language detection** (same pipeline—runs when the grammar checker runs): **Local (langdetect)** uses a small bundled detector to auto-fix wrong `CharLocale` and then grammar-check in the right language; **AI (LLM)** uses your chat API for the same step. Set **Settings → Doc → Sentence language detection**. [Read the Plan](docs/realtime-grammar-checker-plan.md).
...
- **Visualization**: **Viz Helpers** — `quick_plot`, `correlation_heatmap`, `time_series_plot`. Charts insert on the sheet; `=PY()` / `matplotlib` plots work the same way. **Packages:** `pip install matplotlib seaborn`. See [Visualization](docs/enabling_numpy_in_libreoffice.md#visualization).
- **Symbolic Math (SymPy)**: **Math Helpers**.
- **Quantitative Finance**: **Quant Helpers** — `fetch_historical_data`, `technical_analysis`, `portfolio_tearsheet`, `efficient_frontier`. **Packages:** `pip install yfinance pandas-ta quantstats pyportfolioopt`.
- **Optimization & OR**: **Optimize Helpers** — `optimize_portfolio`, `linear_programming`, `solve_scheduling_problem`. See [Optimization](docs/enabling_numpy_in_libreoffice.md#optimization).
- **Unit conversion (Pint)**: **Units Helpers** — `convert_quantity`, `parse_quantity`, `format_quantity`, `check_dimensionality`. On Calc, `convert_quantity` / `parse_quantity` write a **single formatted cell** at the selection anchor by default (e.g. `36 km/h`); pass `output_style: "detailed"` for a key-value grid. Goes beyond Calc's built-in `CONVERT()` for compound units like `m/s` → `km/h`. **Packages:** `pip install pint`. See [Data Engineering / Units](docs/enabling_numpy_in_libreoffice.md#data-engineering-units).
...
- **Local OCR (Writer & Calc)**: Extract text and layout from embedded images offline via **Docling** — no cloud vision API required. **Vision Helpers** — `extract_text` (Writer: inserts at cursor; Calc: sheet report below the image). Settings: **WriterAgent → Vision OCR Settings…** for pipeline defaults; **Settings → Python** for venv path and **Test**. [Image recognition design](docs/image-recognition.md).
...
- **Cross-Document Research**: Say **my** or **our** in the sidebar (e.g. *“pull Q4 from our budget spreadsheet”*) to read other files in the same folder as your saved document; edits stay on the active doc. Cross-file discovery is configurable in **Settings → Embeddings → Cross-file search mode** — **None** (default, **grep_nearby_files**), **Embeddings** (**search_embeddings**), or **SQLite FTS** (**search_nearby_files** over a per-folder BM25 index).
- **Optional folder search cache** (off by default) builds beside your documents in `writeragent_embeddings/` (Writer `.odt`, Calc `.ods`, Impress `.odp`, Draw `.odg`): Chroma vectors for semantic search, or **`fts5.db`** for SQLite FTS5 keyword/proximity search. [Multi-document plan](docs/multi-document-dev-plan.md) · [Embeddings & folder FTS](docs/embeddings.md). **Embeddings mode packages:** `sentence-transformers numpy chromadb langgraph langchain-core langchain-text-splitters envwrap odfpy`.
...
- **Run Python Script**: **Tools → Run Python Script…** — built-in sections **Analysis**, **Viz**, **Math**, **Units**, **Quant**, **Optimize**, **Vision** (set **Data** range where applicable, edit `params` if needed, run). Also available via `run_venv_python_script` and `=PY()` / `=PYTHON()`. Configure venv in **Settings → Python**.
- **Vision / OCR helpers (Writer & Calc)**: **Vision Helpers** — `extract_text`, `extract_structure`. Install: `pip install docling rapidocr-paddle numpy pillow onnxruntime` — **`onnxruntime`** required for default RapidOCR (ONNX); **Settings → Python → Test** confirms **Vision Libraries**. Optional Paddle fallback: `pip install paddleocr paddlepaddle`.
- **Recommended packages**: install `pywebview jedi PyQt6 PyQt6-WebEngine qtpy` for the Monaco editor UI. `numpy` (as `np`), `pandas` (as `pd`), `sympy` (as `sp`), standard library `math`, `datetime`, `re`, `random`, `statistics`, `collections`, `itertools`, `json`, and `csv` are auto-imported to avoid needing manual imports. Optional viz packages: `matplotlib`, `seaborn`. Optional quant packages: `yfinance`, `pandas-ta`, `quantstats`, `pyportfolioopt`. Optional units package: `pint` (Settings → Python **Data Engineering Libraries**). Use the **Test** button in Settings to see which are installed.
...
Trusted **Units Helpers** are now shipped for Writer and Calc: convert, parse, format, and dimensionally-check physical quantities via [Pint](https://github.com/hgrecco/pint). Use **Run Python Script → Units Helpers** (`[Units] convert_quantity`, …), edit `params`, and run. Writer inserts formatted text at the selection; Calc writes a single cell by default (e.g. `36 km/h`) or a detailed key-value grid with `output_style: "detailed"`. Install `pip install pint`; Settings → Python **Data Engineering Libraries** confirms availability. Details: [Data Engineering / Units](docs/enabling_numpy_in_libreoffice.md#data-engineering-units).
