...
- **Visualization**: **Viz Helpers** — `quick_plot`, `correlation_heatmap`, `time_series_plot`. Charts insert on the sheet; `=PY()` / `matplotlib` plots work the same way. Supports returning lists or dicts of figures (multiple images inserted at once). **Packages:** `matplotlib`, `seaborn`. See [Visualization](docs/enabling_numpy_in_libreoffice.md#visualization).
