...
- **Optional folder search cache** (off by default; enable **Embeddings + FTS** in **Settings → Embeddings**) builds beside your documents in `writeragent_embeddings/` (supporting both ODF formats like `.odt`, `.ods`, `.odp`, `.odg` and Office Open XML formats like `.docx`, `.xlsx`, `.pptx`): one **`corpus.db`** (FTS5 + sqlite-vec). Default model is **`paraphrase-multilingual-MiniLM-L12-v2`** for out-of-the-box multilingual support. Cross-file discovery uses **`search_nearby_files`** — keyword (BM25/NEAR) and semantic hits merged with **Reciprocal Rank Fusion** (RRF). [Multi-document plan](docs/multi-document-dev-plan.md) · [Embeddings & hybrid search](docs/embeddings.md). **Venv packages:** `sentence-transformers numpy sqlite-vec langgraph langchain-core langchain-text-splitters envwrap odfpy`.
