...
- **Optional folder search cache** (off by default) builds beside your documents in `writeragent_embeddings/` (Writer `.odt`, Calc `.ods`, Impress `.odp`, Draw `.odg`): unified **`corpus.db`** (FTS5 + sqlite-vec) for semantic and/or keyword search. [Multi-document plan](docs/multi-document-dev-plan.md) · [Embeddings & folder FTS](docs/embeddings.md). **Venv packages:** `sentence-transformers numpy sqlite-vec langgraph langchain-core langchain-text-splitters envwrap odfpy`.
