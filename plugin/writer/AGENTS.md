...
- CJK ruby is portion marks (`RubyText` on the start mark), not a text field. `get_document_content` emits `<ruby>…<rt>…</rt></ruby>` on read; `apply_document_content` does not recreate Ruby portions. Search/`getString()` are base-only. See [docs/writer/ruby.md](../../docs/writer/ruby.md).
