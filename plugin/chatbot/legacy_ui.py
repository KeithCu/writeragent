...
class EditWriterAgentConfigListener(BaseActionListener):
    ...
    def on_action_performed(self, rEvent):
        from plugin.chatbot.external_editor import open_writeragent_json_in_editor
...
def input_box(ctx, message, title="", default="", x=None, y=None):
    ...
    # After execute() returns False (ESC / window close), the toolkit may already
    # tear down the modal dialog; calling dispose() again can segfault (no Python log).
    need_dispose = True
    try:
