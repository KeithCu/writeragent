...
from plugin.networking.client import LlmClient, strip_leaked_chat_template_control_tokens
...
def test_persistent_connections(client):
    with (
        ...
        patch("plugin.networking.client.get_unverified_ssl_context") as mock_ssl,
    ):
...
def test_stream_request_with_tools_tls_retry():
    ...
    with patch("http.client.HTTPSConnection") as mock_https, \
         patch("plugin.networking.client.get_unverified_ssl_context") as mock_unverified_ssl:
...
def test_make_chat_request_includes_dev_build_prefix_when_enabled():
    ...
    with patch("plugin.networking.client.should_prepend_dev_llm_system_prefix", return_value=True):
...
def test_request_with_tools_strips_leaked_control_tokens_in_sync_response(client):
    ...
    with (
        ...
        patch("plugin.networking.client.get_unverified_ssl_context"),
    ):
        mock_conn = MagicMock()
