...
Local Kokoro chooses the phonemizer with `kokoro_g2p_lang` on the utterance (chat replies and the Test voice sample). The voice id stays the one the user selected. Kana uses Misaki `ja`. Han without kana uses `zh`, or `ja` when the UI locale is Japanese. Devanagari uses `hi`. ASCII Latin is treated as English espeak (`en-us`, or `en-gb` for a British voice or en_GB), so an English sentence is not run through Japanese, French, or Spanish Misaki. Accented Latin uses the voice or UI locale when that is es/fr/it/pt. langdetect (in the dev venv) may replace this later and is not imported; ASCII French such as "Bonjour, je suis..." is classified as English until then.

Non-English Kokoro languages are phonemized with [Misaki](https://github.com/hexgrad/misaki) in the Settings → Python venv, then `Kokoro.create(..., is_phonemes=True)`. kokoro-onnx's built-in espeak-ng reads Japanese kanji as the words "chinese letter" (`espeak-ng -v ja -x` has no kanji lexicon) and does not apply Kokoro's phoneme map, so French and Spanish sound wrong too. Japanese uses `misaki[ja]` (`JAG2P`), Chinese uses `misaki[zh]` (`ZHG2P()` legacy frontend; `version="1.1"` is the separate v1.1-zh model), and Spanish, French, Italian, Hindi, and Brazilian Portuguese use `misaki.espeak.EspeakG2P`. English (`en-us` / `en-gb`) stays on espeak-ng so `misaki[en]` (which installs torch) does not change shipped English audio. The first speak of a non-English voice installs the matching extra when it is missing (`uv pip install 'misaki[ja]'`, `'misaki[zh]'`, or `misaki phonemizer-fork espeakng-loader`). If that install fails, speech still runs and may misread the language.
