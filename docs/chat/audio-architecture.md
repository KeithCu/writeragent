...
When the TTS provider is LLM Endpoint and the speech list harvested `supported_voices` for the selected model, Settings → Voice uses those ids (`cached_tts_supported_voices`). That is how Grok and Gemini voices show up without a hand-maintained list. Those rows, and the OpenAI alloy/nova catalog used when a host such as Together has no harvested list, are sorted by display label (case-insensitive) before they go in the combo. A voice that is already in the list stays selected. Local Kokoro (including an endpoint model id that uses the Kokoro catalog) and Piper keep catalog and locale order. A model that is on the OpenRouter speech list but omitted voices, or an OpenRouter endpoint whose speech list has not been fetched yet, leaves the combo editable instead of substituting the OpenAI alloy list. If the saved voice is not in the new list, speech still uses the first advertised id. Opening Settings stores the first row of the sorted combo with `set_scoped_tts_voice` (`audio.tts_voice_openrouter` for those models, so it does not overwrite `audio.tts_voice_openai`). Refreshing TTS models refreshes that voice list; the fetch already fills `_tts_supported_voices`.
