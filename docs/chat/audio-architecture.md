...
**Test voice** sits on the Voice row (`audio__test_voice`, not saved). It speaks `Hello, I'm your LibreOffice WriterAgent.` through `speak_text_async` using the provider, model, voice, speed, and Enable Speech Output checkbox currently on the page (OK has not necessarily written them). The sentence and the button label go through gettext, so they follow the LibreOffice UI locale. For Kokoro, that locale picks Misaki versus English espeak (`kokoro_lang_for_ui_locale`); chat replies still pick the language from the voice id. A locale Kokoro does not support falls back to a kana / CJK-ideograph check, then English. langdetect is not used here. If speech output is off, or playback throws, the dialog stays open and the reason is logged (and shown in a message box).
