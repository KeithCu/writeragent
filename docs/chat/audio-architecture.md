...
Local Kokoro chooses the phonemizer with `kokoro_g2p_lang` on every utterance, including chat replies and the Test voice sample. Latin-script text uses English G2P (`en-us` kokoro-onnx espeak, or `en-gb` for a `b*` voice) and keeps the selected voice, so English on `jf_alpha` or `ff_siwis` is not phonemized with Japanese or French Misaki. Any non-Latin letter keeps today's voice-prefix Misaki language (kana on `jf_*` stays `ja`). langdetect in the dev venv could later split accented French or Spanish from English; it is not imported.
