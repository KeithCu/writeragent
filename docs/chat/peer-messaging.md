...
3. Gateway prepends `[Peer from: Risk memo.odt | uid=… | url=… | peer_ask_id=…]` on the **Calc** session. The inner agent finishes (`specialized_workflow_finished`); Writer **Readys** (no further document_research / python / query in that turn). Caller drain exits.
