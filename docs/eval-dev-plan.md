...
- **Logic**:
  ...
  - **Quadratic IpD Tracking**: Intelligence-per-Dollar estimation using the formula $Value = \text{Correctness}^2 / \text{Cost}$, prioritizing accuracy over low-cost failures.
...
- [x] **Smart Judging**: Integrated multi-dimensional "LLM-as-a-Judge" (Grok 4.1 Fast) with specialized scoring modes (Structural vs. Creative).
- [x] **Gold Standards**: Persistent `gold_standards.json` reference answers generated by a high-tier "Teacher" model (Claude Sonnet 4.6).
- [x] **Value Metric**: Implemented **Quadratic Utility Scoring** ($Value = Correctness^2 / Cost$) to prioritize accuracy over raw cost.
- [x] **Detailed Reporting**: Granular CSV/JSON logging inclusive of Accuracy, Formatting, and Naturalness sub-scores and judge reasoning.
- [x] **Detailed Plan**: Created `EVALUATION_PLAN_DETAILED.md` with scenario archetypes.
...
### D. Multi-Model Sweep
- [x] **Sweep All History**: Automated multi-model benchmarking script (`run_eval_multi.py`) with real-time OpenRouter cost estimation.
