...
| `ci_debug` | Off (default). On: `WRITERAGENT_CI_DEBUG=1` — per-worker nodeid trail, faulthandler dump at 240s, leftover process dump, upload `ci-debug-<os>-<run_id>`. `--max-worker-restart=0` is already on every GHA `make pytest` (`GITHUB_ACTIONS`). Does **not** fix a native hang; it is how we get the crashed-worker nodeid. |
