...
2. **▶ / code field vanished after run** — **fixed**: `update_in_prompt` used to `setString` the whole gutter paragraph, which deleted in-flow `ControlShape`s. It now replaces leading Text portions only (stops at Frame). ▶ sits on the `In [n]:` gutter; the gray TextField is the next paragraph. `apply_run_result` fills an existing empty paragraph under Output when present; it only inserts a `PARAGRAPH_BREAK` when the current para has content, and splits afterwards only if stdout would mash onto the next heading.
