...
| `image_model` | Model id for image generation on the configured endpoint. Writes use `set_image_model()`. OpenRouter’s catalog default (when unset) is `google/gemini-3.1-flash-lite-image`. |
