...
- **Mechanism**: A **hidden temporary Writer** document loads the fragment with the same **`HTML (StarWriter)`** filter and cursor import path as Writer tools; the body is **selected** with a text cursor (not the view cursor—required for hidden docs), then **`getTransferable` → `select(cell)` → `insertTransferable`** on the Calc controller. The temp Writer is opened with **`_blank`** (and `Hidden=True`), not `_default`: headless Windows can reuse `testing_runner`'s hidden keeper frame under `_default` and hang (`test_insert_cell_html`, GHA 33667530529). Step logs (`insert_cell_html_rich: … start/done`) go to **stderr** as well as `writeragent.calc` — GHA 33699746211 timed out with no step line because file-only `log.info` never reached the Actions log. `testing_runner` also prints `KEEPER load/close`, Windows `soffice` PIDs via `tasklist`, and the FAIL exception on `TEST end` (so the three fast PY FAILs are not lost if a later test hangs).
