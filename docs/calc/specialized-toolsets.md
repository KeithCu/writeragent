...
- **Mechanism**: A **hidden temporary Writer** document loads the fragment with the same **`HTML (StarWriter)`** filter and cursor import path as Writer tools; the body is **selected** with a text cursor (not the view cursor—required for hidden docs), then **`getTransferable` → `select(cell)` → `insertTransferable`** on the Calc controller. The temp Writer uses target **`_wa_calc_html`** (FrameSearchFlag `CREATE|GLOBAL`) and `Hidden=True` — not `_default` (GHA 33667530529) and not `_blank` (same target as `testing_runner`'s Windows keeper; 542's switch did not unblock 33699746211). If the load returns an already-open Writer RuntimeUID, that doc is not closed. Step logs go to **stderr** as well as `writeragent.calc`. `testing_runner` prints `KEEPER load/close`, Windows `soffice` PIDs via `tasklist`, `TEST call` / `TEST returned` around the test body, and the FAIL exception on `TEST end`. GHA 33703959362 timed out after `insert_cell_html: execute done` with no `TEST end` — `test_insert_cell_html` now breadcrumbs each post-execute assert / cell read / portion enum, and `@with_native_doc` logs enter / body-returned / teardown for that test only (including `_reset_calc_doc` clearContents). `testing_runner` also arms `faulthandler.dump_traceback_later(90, repeat=True)` to **stderr** (all Python threads) for that test only and cancels it on `TEST end` — not gdb; in-process stacks only; not gated on `WRITERAGENT_CI_DEBUG`. GHA 33707990007 hung in calc native-doc wipe on `doc.isReadonly()` via `set_document_scripts` after `clearContents`; `_clear_writeragent_udprops` now writes `DOCUMENT_SCRIPTS_UDPROP` with `set_document_property` and does not call `isReadonly`.
