...
- **3-Step Workflow**: System prompts guide the LLM to follow a 3-step execution pattern:
  1. Obtain sheet summary / small cell range peek (`get_sheet_summary` or `read_cell_range`). Oversized `read_cell_range` stays capped at 80 cells: peek grid + `rows`/`columns`/`cells`, and a cardinality-capped `column_distincts` summary (high-unique columns are not listed).
  2. Row-wise ordinary Calc formulas go through `write_formula_range` (fill-down adjusts relative refs). Reductions that spill a small result use `=PY` into **one** empty cell **outside** the data — not a hard =PY funnel for every large range.
  3. Provide a brief confirmation referencing modified cell/range addresses.
- **Bulk CSV / Formula Range Writing**: `write_formula_range` performs bulk programmatic `setDataArray`/`setFormulaArray` updates, and can ingest raw CSV string blocks (parsing delimiters automatically) without file I/O overhead. Description order is values / native formulas / fill-down first, `=PY` spill second (anti-husk `to_pandas()` paragraph stays verbatim). The `values` parameter distinguishes a single formula string (1-D fill-down/across; relative refs adjust) from a JSON array (exact per-cell; repeating the same `=B2*…` pins every row). Optional `source` (A1 / `Sheet.A1`) copies that block onto dest `range` (top-left / source extent) via Calc `copyRange` — formulas + values, relative refs adjust; do not pass `values` with `source`. There is no separate `copy_range` tool. `sort_range` teaches reorder-via-tool plus key/direction at the tool (not another CALC_CORE Don't/Do).
...
- **Mechanism**: A **hidden temporary Writer** document loads the fragment with the same **`HTML (StarWriter)`** filter and cursor import path as Writer tools; the body is **selected** with a text cursor (not the view cursor—required for hidden docs), then **`getTransferable` → `select(cell)` → `insertTransferable`** on the Calc controller. After paste, the insert path **releases SystemClipboard if it still owns that transferable** (only when ours — do not clobber an unrelated user copy), then **reuses** the temp Writer on **`_wa_calc_html`** instead of closing it while Calc still holds the paste (GHA 33771766524: clipboard was empty and `close()` returned, but teardown then hung at `desktop.getComponents()`; GHA 33763078357: the same insert + undo + teardown completes). Failed inserts before paste may still close. Reuse clears the body with `setString("")` before each HTML load. Do not call `gc.collect()` on the transferable — GHA 33771769928 aborted in UNO proxy teardown during Draw. The temp Writer uses target **`_wa_calc_html`** (FrameSearchFlag `CREATE|GLOBAL`) and `Hidden=True` — not `_default` (GHA 33667530529) and not `_blank` (same target as `testing_runner`'s Windows keeper; 542's switch did not unblock 33699746211). If the load returns an already-open Writer RuntimeUID (keeper), that doc is not closed. Step logs go to **stderr** as well as `writeragent.calc`. `testing_runner` prints `KEEPER load/close`, Windows `soffice` PIDs via `tasklist`, `TEST call` / `TEST returned` around the test body, and the FAIL exception on `TEST end`. GHA 33703959362 timed out after `insert_cell_html: execute done` with no `TEST end` — `test_insert_cell_html` now breadcrumbs each post-execute assert / cell read / portion enum / a pre-teardown `getDocumentProperties` probe, and `@with_native_doc` logs enter / body-returned / teardown for that test only (including `_reset_calc_doc` clearContents, a RuntimeUID probe before the udprop write, and per-UNO-step traces inside `set_document_property`). `testing_runner` also arms `faulthandler.dump_traceback_later(90, repeat=True)` to **stderr** (all Python threads) for that test only and cancels it on `TEST end` — not gdb; in-process stacks only; not gated on `WRITERAGENT_CI_DEBUG`. GHA 33707990007 hung in calc native-doc wipe on `doc.isReadonly()` via `set_document_scripts` after `clearContents`; `_clear_writeragent_udprops` writes `DOCUMENT_SCRIPTS_UDPROP` with `set_document_property` and does not call `isReadonly` (do not skip that write — GHA 33763078357 relocated the hang to `getDocumentProperties()`).
...
- **WriterAgent (`write_formula_range`)**: **Range-centric bulk writing**. Operates directly via programmatic PyUNO `setDataArray()` and `setFormulaArray()`. Accepts structured rectangular datasets (tuples, 2D arrays, or raw CSV blocks) and commits them in a single transaction. This is highly efficient for generating tables and does not affect the active UI cursor selection. A single ordinary formula into a **1×N or N×1** range fill-down/across with relative A1 adjust (`$` stays absolute) — implemented in Python (`formula_fill.py`), not LO fill/series. `=PY` / PYTHON / PYTHONFUNCTION use a **span-peek**: same-row or single-cell refs outside the quoted Python string adjust; a multi-row DataRange (or unparseable args) stays verbatim so `A1:H1517` is not walked. A 2-D range plus one formula is refused (pass an explicit array).
...
|---------------|--------------------|----------------|-------|
| **Cells** | ✅ Implemented | `cells.py`: `read_cell_range`, `write_formula_range`, `set_style`, `insert_cell_html` ([`rich_html.py`](../../plugin/calc/rich_html.py)), merge/sort/delete helpers | Basic range + style + **HTML → rich text in one cell** ([§ Rich HTML in a single cell](#rich-html-in-a-single-cell)). Full read/write date & time lifecycle (ISO + duration `PT…` wire): [date-time-handling.md](date-time-handling.md). `read_cell_range` returns ISO/`PTnHnMnS` in `value` with `type` / `format_category`; oversized reads stay at 80 cells (peek + size + capped `column_distincts`). `write_formula_range` ingests the same shapes as real Calc serials. 1-D formula fill-down/across + `=PY` span-peek: [`formula_fill.py`](../../plugin/calc/formula_fill.py). Teaching: fill-down for row-wise formulas (one string on `values`, not a JSON array of copied refs); `=PY` for small reductions. `sort_range` teaches reorder + key/direction at the tool. |
| **Ranges** | ✅ Implemented | `cells.py`: Get/SetRangeValues, Get/SetRangeFormulas | — |
| **Sheets** | ✅ Implemented | `sheets.py`, `sheet_filter.py`: ListSheets, CreateSheet, SwitchSheet, GetSheetSummary, `apply_sheet_filter`, `clear_sheet_filter`, `get_sheet_filter` | Basic sheet ops + AutoFilter. `create_sheet` is CRUD-only (tab exists; no cells copied) — populate with `write_formula_range` `source`. Stays specialized. After a sheets hop that created empty tab(s), the gateway forwards the same `instruction` field as web research (`SHEETS_CREATED_NOT_POPULATED_INSTRUCTION`) so the outer can populate. |
