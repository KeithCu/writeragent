...
- `--student scripted` (`scripted_student.py`): no API key; pass is `example_passed` (substring + oracles + process). `-j` is threads. Do not use `tests/eval_runner.py`. Do not set `WRITERAGENT_TESTING=1` for LO eval. The Debug-menu Evaluation Dashboard (`plugin/chatbot/eval_dashboard_ui.py`) still lazy-imports `tests.eval_runner.run_benchmark_suite`. `make build` ships `tests/`; release (`--no-tests`) omits it and strips that menu. If Run is reached without the module, the dialog reports that the benchmarks are not in the build and points at `scripts/prompt_optimization/run_eval.py`.
