...
- **Prefix dedup**: Within each `(doc_id, locale)` group, if item A's slice text is a **proper prefix** of item B's (e.g., typing "This is" → "This is a" → "This is a story."), the shorter items are dropped. Additionally, items with the same `inflight_key` (same text fingerprint) are superseded by sequence number (only the highest `enqueue_seq` survives). This is combined with a 1-second pause mechanism: the worker collects requests and waits until there is a 1-second period of no new requests before processing the batch, ensuring checks only run when the user stops typing.
...
- Unit (work queue dedup): [`plugin/tests/test_grammar_work_queue.py`](../plugin/tests/test_grammar_work_queue.py) — prefix dedup, supersede, mixed dedup, cross-locale independence, trailing whitespace normalization, per-sentence cache roundtrip.
