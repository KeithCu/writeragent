...
- **2026-04-27**: Replaced per-sentence `run_in_background` + debounce-sleep pattern with single sequential `_GrammarWorkQueue` (`queue.Queue` + one daemon thread). Added `deduplicate_grammar_batch` with prefix dedup + sequence-based supersede. Removed `GRAMMAR_WORKER_DEBOUNCE_MS`, `_INFLIGHT_JOBS`, `_wait_for_inflight_job`. `GrammarWorkItem` and dedup live in [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py) for UNO-free unit testing.
...
- **2026-05-10 (consolidate)**: Merged JSON, BreakIterator helper, sentence split, and offset normalization into [`grammar_proofread_text.py`](../plugin/writer/locale/grammar_proofread_text.py) to avoid many tiny files; cache unchanged.
- **2026-05-10 (queue consolidate)**: `GrammarWorkItem`, batch dedup, and enqueue supersede / stale helpers merged into [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py) (removed `grammar_proofread_work_item.py`, `grammar_queue_state.py`).
