...
- **Pure Python modules**: [`grammar_proofread_locale.py`](../plugin/writer/locale/grammar_proofread_locale.py) — **`GRAMMAR_REGISTRY_LOCALE_TAGS`**, UNO `Locale` ↔ BCP-47 bridging, English names for prompts; Unicode sentence terminals / trailing closers, `looks_complete_sentence`, abbrev table + Thai/Lao/Khmer whitespace chunking, scheduling thresholds, **`GRAMMAR_PROOFREAD_*`** caps, system prompt template, worker pause timeout, `parse_grammar_json`. [`grammar_proofread_text.py`](../plugin/writer/locale/grammar_proofread_text.py) — BreakIterator orchestration, `split_into_sentences`, offset normalization, **sentence scheduling** (`candidate_sentence_spans_for_proofreading`, overlap/threshold filters). [`grammar_proofread_cache.py`](../plugin/writer/locale/grammar_proofread_cache.py) — sentence LRU + ignore rules. [`grammar_proofread_work_item.py`](../plugin/writer/locale/grammar_proofread_work_item.py) — `GrammarWorkItem`, batch dedup. [`grammar_queue_state.py`](../plugin/writer/locale/grammar_queue_state.py) — pure enqueue supersede / stale helpers. [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py) — `GrammarWorkQueue`, `run_llm_and_cache`.
...
- **2026-04-26**: Registry expanded to all shipped UI translation locales; BCP-47 registry + normalized `CharLocale` for cache/LLM (today: [`grammar_proofread_locale.py`](../plugin/writer/locale/grammar_proofread_locale.py)).
...
- **2026-04-27**: Replaced per-sentence `run_in_background` + debounce-sleep pattern with single sequential `_GrammarWorkQueue` (`queue.Queue` + one daemon thread). Added `deduplicate_grammar_batch` with prefix dedup + sequence-based supersede. Removed `GRAMMAR_WORKER_DEBOUNCE_MS`, `_INFLIGHT_JOBS`, `_wait_for_inflight_job`. `GrammarWorkItem` and dedup live in [`grammar_proofread_work_item.py`](../plugin/writer/locale/grammar_proofread_work_item.py) for UNO-free unit testing.
...
- **2026-04-28**: Implemented hybrid sentence splitting in [`grammar_proofread_text.py`](../plugin/writer/locale/grammar_proofread_text.py). Switched from a pure-Python regex to LibreOffice's `BreakIterator` (for proper quote/ellipsis handling) augmented with a custom abbreviation heuristic, while retaining whitespace chunking specifically for `th`/`lo`/`km`. Moved sentence splitter tests to the UNO native test suite.
...
- **2026-05-10 (terminals)**: Unicode STerm-style sentence terminals and Pe/Pf trailing closers live in [`grammar_proofread_locale.py`](../plugin/writer/locale/grammar_proofread_locale.py); [`ai_grammar_proofreader.py`](../plugin/writer/locale/ai_grammar_proofreader.py) and [`grammar_proofread_cache.py`](../plugin/writer/locale/grammar_proofread_cache.py) import `looks_complete_sentence` from there so cache eviction and proofreader gating cannot drift.
- **2026-05-10 (locale module)**: Added [`grammar_proofread_locale.py`](../plugin/writer/locale/grammar_proofread_locale.py) — consolidates terminals, abbrev/Thai chunking, worker caps/prompt/pause, and `parse_grammar_json`; cache module is LRU-only for sentence tables.
