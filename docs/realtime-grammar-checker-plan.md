...
- **Why not spaCy / LLM / full per-locale dumps**: spaCy vocab dumps were rejected (PII and extraneous tokens). Invented hand lists for pl/sv/uk/etc. are not used — CLDR is the only static table. CLDR does **not** cover all ~36 grammar registry tags; heuristics remain the fallback. Embeddings already use ICU `@ss=standard` (same CLDR data) separately; LO UNO BreakIterator does not apply those filters, which is why this whitelist post-check exists.
