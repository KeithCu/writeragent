...
- **Newest-wins dedup + stale suppression**: Within each `(doc_id, locale)` group, **prefix-related conflicts now keep the newest request** (`enqueue_seq`), not the longest text. Items with the same `inflight_key` are also superseded by sequence number (highest survives). **`inflight_key`** is **`{aDocumentIdentifier}|{locale}`** (no slice text fingerprint): mid-sentence edits are not prefix-related, so a per-slice fingerprint would give every keystroke a unique key and defeat same-key supersede, prefix dedup, and `_latest_seq` stale checks. `_GrammarWorkQueue` additionally performs a **pre-execute stale check** against `_latest_seq` and skips any survivor older than the latest known sequence for that key, which closes cross-batch race windows. This is combined with a 1-second pause mechanism: the worker collects requests and waits until there is a 1-second period of no new requests before processing the batch, ensuring checks only run when the user stops typing.
