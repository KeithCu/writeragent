...
- **`doProofreading`** (async return path): On a **cache miss**, WriterAgent **returns immediately** with empty `aErrors` and schedules (or joins) a **background** LLM worker. It **does not** wait inside `doProofreading` or pump `processEventsToIdle()` for results. That keeps **menus and chrome responsive** while grammar runs; underlines appear on a **later** Writer proofreading pass once the **sentence cache** is filled. This trades same-call squiggles for full UI responsiveness (sync wait caused partially dead menus during long checks).
...
- **Concurrency**: at most one in-flight LLM worker per sentence-level inflight key; duplicate calls for the same in-flight work return empty without blocking.
- **Worker debounce (hard-coded)**: On the **background thread only**, the worker sleeps `GRAMMAR_WORKER_DEBOUNCE_MS` (fixed in code, not Settings) then checks a per-key sequence so a **newer** schedule for the same sentence **supersedes** an older worker before the LLM runs—coalescing API traffic while typing without blocking the UI.
