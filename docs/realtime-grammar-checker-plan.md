...
**Regression test:** [`test_two_sentences_string_prefix_collision_both_survive`](../plugin/tests/writer/locale/test_grammar_work_queue.py). Implementation notes are in **comments directly above** [`deduplicate_grammar_batch`](../plugin/writer/locale/grammar_proofread_work_item.py) in that file (not in the module docstring).
...
- **Pure Python modules**: [`grammar_proofread_text.py`](../plugin/writer/locale/grammar_proofread_text.py) — BreakIterator, sentence splitting, LLM JSON parse, offset normalization, **sentence scheduling** (`candidate_sentence_spans_for_proofreading`, overlap/threshold filters next to `split_into_sentences`; not in cache — avoids import cycles). [`grammar_proofread_cache.py`](../plugin/writer/locale/grammar_proofread_cache.py) — sentence LRU; **authoritative** Unicode sentence-terminal / trailing-closer sets and `looks_complete_sentence`. [`grammar_proofread_work_item.py`](../plugin/writer/locale/grammar_proofread_work_item.py) — `GrammarWorkItem`, batch dedup. [`grammar_queue_state.py`](../plugin/writer/locale/grammar_queue_state.py) — pure enqueue supersede / stale helpers. [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py) — `GrammarWorkQueue`, `run_llm_and_cache`, caps/prompt constants. [`grammar_proofread_engine.py`](../plugin/writer/locale/grammar_proofread_engine.py) — compatibility shim re-exporting **`GRAMMAR_REGISTRY_LOCALE_TAGS`** only (same symbol as [`grammar_locale_registry.py`](../plugin/writer/locale/grammar_locale_registry.py)).
...
- **All settings (Doc tab)**: `doc.grammar_proofreader_*` in [`plugin/doc/module.yaml`](../plugin/doc/module.yaml) — enable (default **off**), wait timeout (ms), optional model (empty = same as chat `text_model`), and `doc.grammar_proofreader_pause_during_agent` (default **off**) to pause grammar API requests while sidebar chat/agent work is active. LLM max output tokens (512) and the **pathological** slice ceiling **`GRAMMAR_PROOFREAD_SAFETY_MAX_CHARS`** are **fixed in code** in [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py). The Doc tab also inlines Calc’s **Max Rows Display** (`calc.max_rows_display` via `config_inline: doc` in [`plugin/calc/module.yaml`](../plugin/calc/module.yaml)).
...
**Terminology.** The shipped code uses a **global integer counter** (`next_enqueue_seq()` / `_ENQUEUE_SEQ` in [`grammar_work_queue.py`](../plugin/writer/locale/grammar_work_queue.py)), incremented when a cache miss enqueues work; each [`GrammarWorkItem`](../plugin/writer/locale/grammar_proofread_work_item.py) stores it as **`enqueue_seq`**. This is **not** the same as `time.monotonic()` — that clock is used elsewhere only for **elapsed milliseconds** on LLM requests (status/diagnostics), not for ordering queue items.
...
- **2026-05-10 (refactor)**: Split engine into submodules; added [`grammar_queue_state.py`](../plugin/writer/locale/grammar_queue_state.py) for testable enqueue supersede / stale checks; [`grammar_proofread_engine.py`](../plugin/writer/locale/grammar_proofread_engine.py) remains the stable import surface. Unit tests: [`test_grammar_queue_state.py`](../plugin/tests/writer/locale/test_grammar_queue_state.py).
- **2026-05-10 (consolidate)**: Merged JSON, BreakIterator helper, sentence split, and offset normalization into [`grammar_proofread_text.py`](../plugin/writer/locale/grammar_proofread_text.py) to avoid many tiny files; cache, work item, and queue state modules unchanged.
- **2026-05-10 (terminals)**: Unicode STerm-style sentence terminals and Pe/Pf trailing closers live only in [`grammar_proofread_cache.py`](../plugin/writer/locale/grammar_proofread_cache.py); [`ai_grammar_proofreader.py`](../plugin/writer/locale/ai_grammar_proofreader.py) imports `looks_complete_sentence` so cache eviction and proofreader gating cannot drift.
