...
| **TD4** | Over-clever queue logic | The tail-replace dedup (reaching into `queue.Queue` internal `_q.mutex` + `deque`), `inflight_key` + `enqueue_seq` scheme, and multi-layer stale suppression is powerful but has a long bug history (see multiple appendices). It is the highest cognitive-load code in the grammar system. | `grammar_work_queue.py` (enqueue, `deduplicate_grammar_batch`, stale guards) | Medium (if touched carelessly) | High (long-term) | **Completed** — (a) Added a comprehensive mental-model docstring to `grammar_work_queue.py` explaining all three dedup layers, `inflight_key` design, `enqueue_seq` as generation stamp, and the dict/`deduplicate_grammar_batch` redundancy rationale. (b) Extracted `should_replace_for_key` (shared by `deduplicate_grammar_batch` and `_drain_loop`) and `filter_stale_and_group` (replaces inline stale-filter + grouping in `_drain_loop`) as pure testable functions. Removed unused `_latest_seq_for` method. (c) Evaluated `InflightTracker` wrapper and rejected it — `GrammarWorkQueue` is small enough that an extra indirection adds more cognitive load than it removes; decision documented in the class docstring. Added 7 new unit tests covering the extracted functions. |
