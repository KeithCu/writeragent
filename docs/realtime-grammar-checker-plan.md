...
| **TD4** | Over-clever queue logic | The tail-replace dedup (reaching into `queue.Queue` internal `_q.mutex` + `deque`), `inflight_key` + `enqueue_seq` scheme, and multi-layer stale suppression is powerful but has a long bug history (see multiple appendices). It is the highest cognitive-load code in the grammar system. | `grammar_work_queue.py` (enqueue, `deduplicate_grammar_batch`, stale guards) | Medium (if touched carelessly) | High (long-term) | **Completed** — (a) Added a comprehensive mental-model docstring... (b) Extracted pure helpers... (c) Evaluated `InflightTracker`...  **Follow-up simplification (2026-05)**: Removed the remaining Layer 1 tail-replace entirely (the mutex poke + direct deque mutation) per the insight documented in the mental model itself. `enqueue()` is now a normal `put()`. Layer 2 drain dict + canonical dedup + Layer 3 guards are the complete, simpler story. Updated module docstring + tests + this plan. and rejected it — `GrammarWorkQueue` is small enough that an extra indirection adds more cognitive load than it removes; decision documented in the class docstring. Added 7 new unit tests covering the extracted functions. |
