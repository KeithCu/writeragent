...
| **CJK ruby (furigana)** | **Read** rewrites glued/dropped ruby to `<ruby>…<rt>…</rt></ruby>`. **Write** strips `<rt>` then sets `RubyText` on the base ([ruby.md](ruby.md)). | Search still sees base only. | — |
