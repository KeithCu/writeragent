...
- **LibreHarper status bar**: LibreHarper paints via `XStatusIndicator`, not the sidebar. Routine per-keystroke results (`Harper`, `Harper check`, `queued`, `N issue(s)`) are not painted — they flashed the bottom bar on every `doProofreading` and could stall typing. Progress (binary download, starting harper-ls) and failures still paint. Frame resolve never creates `com.sun.star.frame.Desktop` (`get_active_document` / `get_desktop`): uno.bin during unopkg enable has no VCL and that create SEGVs. No usable frame → skip paint.
