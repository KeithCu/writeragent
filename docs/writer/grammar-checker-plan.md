...
3. **LLM / queued providers:** Check and return cached sentence errors for **all** sentences in the paragraph immediately. This ensures that as soon as any background check completes and writes to the cache, its errors are returned to LibreOffice on the very next keystroke/call (even if the user has moved on to typing a subsequent sentence). Writer honors those extra `aErrors` (it does not ignore out-of-window marks). **Harper:** Cache lookup and return cover only the **active** sentence(s) this call asked about. The fast path already returned sentence 1's errors when Writer asked about sentence 1, so re-sending them on the sentence-2 call is redundant. When `nStartOfSentencePosition == 0`, the active window is the whole paragraph (load/paste).
4. For uncached sentences, only enqueue background work (or Harper-lint) for the **active** sentence currently being edited (overlapping the cursor), avoiding redundant checking of other sentences.
...
- **Harper fast path and startup warmup**: If the provider is Harper and `harper-ls` is already running in-process, `doProofreading` lints uncached **active** sentences immediately (no queue) and returns only those sentences' cached + freshly linted errors. If the binary is missing or the process is not up, it returns empty/partial errors and starts **one** background ensure (`idle → resolving → ready`, or `failed` with a 30s cooldown). On success, ensure-ready emits "Harper ready" and posts **one** `PROOFREAD_AGAIN` so Writer re-walks after a download-time empty pass. `broadcast_proofread_again` no-ops when no `XLinguServiceEventListener` is hooked yet (common on first session); it remembers the miss, and the first `addLinguServiceEventListener` recovers exactly one re-walk if Harper is already READY or a broadcast was pending. On startup / extension initialization (`maybe_start_harper_async`), both LibreHarper (`HarperProofreader.__init__`) and WriterAgent (`OnStartApp` / proofreader init when in Harper mode) initiate background loading/warmup of `harper-ls` asynchronously so it is ready before the user types. Further proofreading calls while `resolving` do not download again. `max_in_flight` stays LLM-only. `HarperLSClient.close()` closes stdin/stdout then terminate/kill (no blocking LSP shutdown write). Tests call `shutdown_harper_runtime()` on teardown so a leaked `harper-ls` cannot hold an xdist worker. Unit tests that construct `HarperProofreader` must mock `maybe_start_harper_async` — `__init__` warms up when `user_config_dir` is set (the pytest config fixture supplies a real temp dir).
- **`doProofreading`** (async return path): On a **full cache miss**, WriterAgent returns with empty `aErrors` and enqueues a work item. On a **partial cache hit** (some sentences cached, some not), it **returns the cached errors immediately** (better than empty — squiggles appear for already-checked sentences) and enqueues for the remaining uncached sentences. On a **full cache hit** all errors are returned directly, no enqueue needed. For **LLM / queued** providers it **does not** wait inside `doProofreading` or pump `processEventsToIdle()` for results. That keeps **menus and chrome responsive** while grammar runs.
- **Harper READY wait + PE2I**: When `harper-ls` is already in-process, the sync fast path still waits for lint (result semantics unchanged) but the blocking LSP wait runs on a dedicated worker; the linguistic thread does **not** hold `_HARPER_LOCK` across that wait or across `process_events_to_idle`. It pumps `process_events_to_idle(ctx, force=False)` every ~75ms until the result or `_LINT_BUDGET_SEC` so **typing stays live**. Menus may stay dull while the proofread call is open (known Lightproof-era tradeoff). Nested `doProofreading` / `harper_try_lint` while a wait is active fail soft (`None` / empty this walk) and emit one WARN + obs `harper_wait_reenter` (thread, wait age ms, provider) so we can see if LibreOffice actually re-enters. Missing `ctx` falls back to a blocking wait (no pump). LLM enqueue is unchanged.
- **Recheck this document** (Doc tab): dumps **all of L1** and **this document’s L2** (active window under Settings, matched by RuntimeUID), then broadcasts `PROOFREAD_AGAIN` from the live proofreader (`XLinguServiceEventBroadcaster`). LibreOffice’s event has no document id, so other open Writer docs may get another linguistic pass; they refill from **their** L2 (not a full LLM recheck). Model/engine switch stays lazy until the user hits Recheck.
...
- **LibreHarper status bar**: LibreHarper paints via `XStatusIndicator`, not the sidebar. Routine per-keystroke results (`Harper`, `Harper check`, `queued`, `N issue(s)`) are not painted — they flashed the bottom bar on every `doProofreading` and could stall typing. Progress (binary download, starting harper-ls) and failures still paint, **always** via `post_to_main_thread` (never inline even when `on_main_thread()` looks true — linguistic workers can look like VCL while holding `_HARPER_LOCK`). Frame resolve never creates `com.sun.star.frame.Desktop` (`get_active_document` / `get_desktop`): uno.bin during unopkg enable has no VCL and that create SEGVs. Detection lives in `uno_context.desktop_create_is_unsafe()` (process image / `--singleaccept`, not only `sys.argv` — pythonloader can rewrite argv). `get_desktop` fail-softs on those helpers. `HarperProofreader.__init__` also skips harper-ls warmup and the weekly update-check schedule during register/enable so those paths never look up `theDesktop` or call `msgbox`→`get_desktop`. No usable frame → skip paint.
- **LibreHarper packaging (`make deploy-harper`)**: the slim allowlist in [`scripts/libreharper_bundle_paths.py`](../../scripts/libreharper_bundle_paths.py) must include [`module_base.py`](../../plugin/framework/module_base.py). `deploy-harper` leaves WriterAgent installed; `plugin.extend_path` can then run WriterAgent's `plugin/chatbot/__init__.py` (which imports `ModuleBase`) against LibreHarper's regular `plugin.framework` package. The weekly update check is scheduled from `HarperProofreader.__init__` via `plugin.chatbot.extension_update_check` and is wrapped in a warning `except`; that miss is not by itself a Writer vanish. Do not ship a stub `plugin/chatbot/__init__.py` — it would shadow WriterAgent's `ChatbotModule` in a dual install.
- **Harper lint `None`**: `harper_try_lint` never returns `None` silently. Ready-client exceptions and a READY-but-dead process log **ERROR**; empty profile dir logs **ERROR**; idle/resolving/failed walks emit DEBUG obs `harper_try_lint_none`.
- **Harper slow-result WARN**: `_lint_with_client` (used by `harper_try_lint` / `run_harper_lint`) times lint + normalize into errors. If elapsed **>500ms**, one WARN `[harper] slow result elapsed_ms=… text_len=… errors=… cache=miss` so LibreHarper WARN-only logs still show slowness. Sub-500ms results are silent.
- **Result-window obs**: each `doProofreading` that builds a return emits **one** DEBUG `do_proofreading_result_window` **after** cache lookup **and** the Harper fast path (`grep '[grammar] obs do_proofreading_result_window'`). `stage='final'` so this is not a pre-path snapshot. Fields: Writer `n_start` / `n_behind` / `n_next`; `n_errors` (Python list) vs `n_aErrors` (`len(a_res.aErrors)` after UNO conversion); `source` (`cache` / `harper_fast` / `enqueue` / `no_active_spans`); paragraph vs active span counts; window classifier (`in` / `before` / `after` / `straddle`, `error_spans` is `start+len:where` CSV); `rule_ids` (first 8, then `+N`) when non-empty. Headed QA: `n_errors=0 source='harper_fast'` is empty in-process lint; `n_aErrors>0` with no underlines is a Linguistic paint drop. Writer still paints `aErrors` outside that window; the classifier is debug observation only. `before_window > 0` on a later-sentence call means earlier-sentence hits were included in this return (LLM/queue path). Cache-miss state before Harper is `do_proofreading_cache_partial_hit` (`cache_error_count`, not final `n_errors`).
