...
- **Harper fast path**: If the provider is Harper and `harper-ls` is already running in-process, `doProofreading` lints uncached sentences immediately (no queue). If the binary is missing or the process is not up, it returns empty/partial errors and starts **one** background ensure (`idle → resolving → ready`, or `failed` with a 30s cooldown). Further proofreading calls while `resolving` do not download again. `max_in_flight` stays LLM-only.
