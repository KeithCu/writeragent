...
*   **Secondary paths:** Web research and librarian checkbox flows now share `get_chat_response_format_instructions()` → [`RICH_CHAT_SIDEBAR_INSTRUCTIONS`](../plugin/framework/constants.py) (tag list + `HTML_FRAGMENT_RULES` + chat examples; respects `rich_text_control_sidebar`) and post-stream `finalize_sidebar_assistant_response()` / HTML rerender with main chat. Other direct puts in `send_handlers.py` (image results, etc.) and `acp_backend.py` remain on the global audit list.
