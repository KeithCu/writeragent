...
`PYTEST_WORKERS=0` (or empty) drops `-n` for a single process. Local `make pytest` does **not** set `WRITERAGENT_PYTEST_PROGRESS` (dots only). PR CI sets `WRITERAGENT_PYTEST_PROGRESS=1` in `.github/workflows/pr-ci.yml` so collection/count heartbeats print as full stderr lines (pytest/xdist otherwise rewrite one `\r` status line). When that env is on, count lines are every 100, then every 10 after 7800 plus a 15s idle re-announce of leftover inflight nodeids so the last 1% (~7988 items) is not a silent gap (PR #823 GHA 35666812986 / 35669866244). Collection must stay clean of live LibreOffice: `--ignore-glob='*_uno.py'` in pyproject `addopts` so this invocation does **not** collect `plugin.testing_runner` / `*_uno.py` / soffice suites. `tests/conftest.py` still drops leftover `@native_test` functions in mixed modules. Parallel tests may only write under `tmp_path`, the autouse config temp dir, or a `worker_id`-namespaced path; `MagicMock/` cleanup is controller-only. Do not put UNO / `testing_runner` under xdist.
...
`make test-run` is: `make pytest`, then `lo-kill`, then `PYTHONUNBUFFERED=1 $(LO_PYTHON) -u -m plugin.testing_runner` (serial UNO; do not parallelize). The runner prints flushed `SUITE start` / `TEST call` / `TEST returned` / `TEST end` lines on stderr so a glibc `soffice.bin` abort still names the last native test. Pid dumps, `TEST start`, bootstrap, and soak iters print only when `WRITERAGENT_CI_DEBUG=1`. `test_read_range_format_info_performance` uses a 40×40 grid (not 100×100) so the default suite does not copy tens of 10k-cell PyUNO arrays.
...
PR #823 leftover heartbeats (`test_edited_context` / `test_small_fixes_r5` dry_run, ~80s then Make Terminated) were **not** an xdist deadlock and not a job timeout. `pull_request` checks out `refs/pull/N/merge` (PR + `master`). `workflow_dispatch` checks out the branch SHA. Master’s `hyperlink_fixup` portion walk used `if not enum.hasMoreElements()`; MagicMock returns a truthy mock, so three `ApplyDocumentContent` unit tests spun until GHA SIGTERM. `_enum_has_more` (`is True` only) is the product stop. Heartbeats / `--max-worker-restart=0` only named the leftovers.
