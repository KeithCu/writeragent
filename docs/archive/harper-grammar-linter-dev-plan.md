...
| **Thread safety for shared LSP client** | `_HARPER_CLIENT_CACHE` + `HarperLSClient` share one LSP subprocess per binary path. Parallel `lint()` calls would interleave stdin/stdout framing. | Lock serializes UNO `doProofreading` vs one background ensure / leftover drain thread. Harper is never multi-flight (`max_in_flight` is LLM-only). |
| **Ready-process fast path** | Skip the grammar queue when `harper-ls` is already alive; otherwise return empty and start one download/start job (anti-stampede state machine). | Experimental (`harper_try_lint` / `harper_ensure_ready_async`). |
