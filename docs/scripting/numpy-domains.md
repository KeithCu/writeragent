...
- **Showcase Demo (Pretty Dashboard & Executive Layout):** [`tests/fixtures/python_showcase_demo.ods`](../../tests/fixtures/python_showcase_demo.ods) / [`python_showcase_demo.xlsx`](../../tests/fixtures/python_showcase_demo.xlsx) ([`docs/python-in-calc-showcase.md`](python-in-calc-showcase.md)). Modern styled executive dashboard with KPI cards, formatted tables, live `=PY()` / `=PYTHON()` formulas across Sales, Marketing ML, Time Series Forecasting, Portfolio Optimization, Engineering Units, Plot Gallery, and **SQL/DuckDB** (SQL only in cells; RESULTS are a short `=PY()` that reads an explicit SQL cell/range as `data[1]` plus named-range identity as `data[0]`; sheet-only uses `session_duckdb()`; sibling ACS `zip_income.csv` join is live `run_sql` + `scoped_dir`). XLSX named ranges are absolute (`Sales_Analytics!$A$4:$J$39`) so `=PY()` does not pack a calling-cell-shifted `data[0]`. Both formats share the same standard-sheet chrome (title `📊 {Sheet} — {sub}`, blank row 2, section banner row 3, data header / named-range anchors at row 4 — `A4:J39` / `A4:G24`). ODS formulas use single-pass OpenFormula conversion (`[$Sheet.A4:.I39]`) plus TableColumn / TableRow sizes and `table:covered-table-cell` after each `number-columns-spanned` so headed Calc keeps Overview KPI cards and the capability matrix (odfpy spans alone collapse later cells into column A). Spacer rows include an empty cell so they are not self-closed and dropped (which shifted Order_ID off A4). Regenerate via `python scripts/generate_pretty_demo_spreadsheet.py --format all` (also copies [`zip_income.csv`](../../tests/fixtures/zip_income.csv)).
