...
| IDL / RDB | `org.extension.writeragent.PythonFunction.XPythonFunction` | Same IDL module path as WriterAgent ([`extension-core/idl/XPythonFunction.idl`](../extension-core/idl/XPythonFunction.idl)); rebuild via `make rdb-core` (needs `unoidl-write` from `libreoffice-dev` on Ubuntu / Debian, or `libreoffice-fresh-sdk` on Arch). [`scripts/rebuild_librepy_rdb.sh`](../../scripts/rebuild_librepy_rdb.sh) (via [`scripts/find_unoidl.sh`](../../scripts/find_unoidl.sh), shared with [`scripts/rebuild_xprompt_rdb.sh`](../../scripts/rebuild_xprompt_rdb.sh)) also searches PATH, `$OO_SDK_HOME`, macOS `LibreOffice.app`, and Windows `Program Files`; if the SDK is missing it reuses the committed [`extension-core/XPythonFunction.rdb`](../../extension-core/XPythonFunction.rdb) (macOS/Windows CI). |
