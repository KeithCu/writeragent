...
**Code fields (in-flow):** `TextField` inside `ControlShape`, **`AS_CHARACTER`**, `insertTextContent` at document end — same pattern as [`FormCreateControl`](../plugin/writer/specialized/forms.py). ▶ lives on the `In [n]:` gutter paragraph, not in the field’s line box (sharing that box put the 3.2 mm button under the gray rectangle). Height is source line count × 450 HMM plus pad **plus half a line** so a leftover wrap is not clipped without a full empty gray row under short cells — no wrap-width calculator, no HScroll. Cap is about one page body, not a 9 cm box. Models are also registered on the document **draw page** (used to find controls for ▶ wiring and for `read_code_from_field`). The completion dialog’s “fields” count is ▶ + TextField together (one pair per code cell, not duplicates). Re-run must not `setString` the **next** cell’s gutter or field: `nb_out_*` home is *this* cell’s field, not the following `In [n]:` gutter.
