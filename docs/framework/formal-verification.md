...
- **A callee `@deal.pre` does not bound the wrapper.** CrossHair `cover` walks every top-level function; `_normalize_excel_placeholders` being `str_bounded` did not stop `_find_xl_calls` / `rewrite_excel_code` taking raw `str` (issue 469 leftovers). Same for `_split_passage_whitespace_to_sentences` vs `split_passage_to_sentences` in [`embeddings_split.py`](../../plugin/embeddings/embeddings_split.py) (regular cover ~80s / 1113 examples). Put the same length (and alphabet) pre on the wrapper. Check-all run [33928341275](https://github.com/KeithCu/writeragent/actions/runs/33928341275) nested-failed `_find_xl_calls('\t"')` → `_skip_string` (`_deal_excel_src_ok` was len 2 while `_DEAL_REWRITE_SRC` is 1), `convert_model_to_dag` → `convert_cell_to_dag` (scripts pre passed; cell `deps` length did not); check-all [33935176527](https://github.com/KeithCu/writeragent/actions/runs/33935176527) then nested-failed `convert_model_to_dag(scripts=['\x00'])` → `rewrite_excel_code` (`_deal_convert_scripts_ok` was `str_bounded` while `_deal_excel_src_ok` rejects NUL under CrossHair — scripts now share `_deal_excel_src_ok`), `split_passage_to_sentences(..., locale='en@ss=standard')` (`==` DEFAULT allowed a synthesized str that icu4py rejected; CrossHair locale pre is `is DEFAULT_SENTENCE_LOCALE`), and `split_passage_locale_runs_to_chunk_meta` → `split_passage_to_chunk_meta` / `_split_prose_passage_to_spans` (wrapper now shares the ascii meta-value bound and, under CrossHair, only `None` run/default locales so a BCP-47 tag cannot become a new ICU string that fails `is DEFAULT_SENTENCE_LOCALE`). `convert_cell_to_dag` must not require in-range `script_index` when the body fail-closes OOR. `rebuild_python_formula*` slack is `DEAL_MAX_SOURCE+256` (dtype=float growth); overflow tests must use `+256+1`, not `+1`.
- **`convert_model_to_dag` dep regex / `_` placeholder:** Check-all [33940151004](https://github.com/KeithCu/writeragent/actions/runs/33940151004) `PatternError: unterminated character set at position 0` on `convert_model_to_dag(ExcelWorkbookModel(scripts=[''], cells=[ExcelPyCell(..., deps=['_'], ...)], ..., source_path='\x00'), best_effort=True)`. **Not** the #599 NUL-scripts hole (`PreconditionFailed` / alphabet): CPython already fail-closes `resolve_dep('_')` as unrecognized; CrossHair's relib re-parses `_TABLE_ALL_RE`'s `\[#All\]` and `_RANGE_RE`'s `[^']` / `[A-Za-z_]` when a 1-char `_` (legal under `ascii_bounded`, prefix of `_xlfn.`) reaches `re.match`. Gate each regex on a cheap surface shape (`ANCHORARRAY(`, `[#All]`, trailing `#`, A1/range) so junk never hits `re.match`; CrossHair `_deal_convert_cell_ok` now requires exact two-char A1 (`A1`…`Z9`; see 33954468213). Pytest stays `ascii_bounded` so `_xlfn.ANCHORARRAY` / `Table[#All]` remain legal. Do **not** `# crosshair: off` `convert_model_to_dag`.
- **`convert_model_to_dag` single-letter dep / nested note pre:** Check-all [33954468213](https://github.com/KeithCu/writeragent/actions/runs/33954468213) `PreconditionFailed` on nested `_normalize_bindings` (deal message quotes the first clause `type(resolved) is list`) for `convert_model_to_dag(..., deps=['M'], ...)`. **Not** the 33940151004 PatternError hole: `resolve_dep('M')` is unresolved with note `unrecognized dep 'M'` (len ≫ `_DEAL_NOTE_LEN=1` under CrossHair). CrossHair convert deps are exact two-char A1 so `resolve_dep` returns `kind=range` with an empty note. Pytest still allows len-1 junk. Do **not** `# crosshair: off` `convert_model_to_dag`.
...
| `str_bounded(s, max_len, min_len=0)` | Open text: gettext `_()` (`DEAL_MAX_MSGID`), HTML, `=PY()` / Excel source (`DEAL_MAX_SOURCE`), specialize `task`. Length still bounds CrossHair |

`DEAL_MAX_CELL_REF` is **256** under pytest/debug so sheet-qualified live ranges (`Sample Size Calculation.A1:K…`) fit; CrossHair stays **4**. Do **not** put `isascii` on `_()` (msgids include `"✓ Copied!"`, `"Testing…"`) or formula *source*. `strip_html_tags` uses ASCII under `WRITERAGENT_CROSSHAIR=1` and Unicode `str_bounded` under pytest (café body text). `_()` keeps `DEAL_MAX_MSGID` (**1024** in both profiles) so real UI strings survive import under `WRITERAGENT_CROSSHAIR=1`; check-all does **not** shrink that constant — it marks `_()` `# crosshair: off` instead (analyzing 1024-char Unicode msgids would dominate deep check). Pytest `DEAL_MAX_SOURCE` is **8192** (real `=PY()` / Excel scripts, and specialize `task` sanity); CrossHair stays **16**. `_truncate_delegate_task` must `str_bounded` the **full** task at that cap (CrossHair input floor is **1**); `DELEGATE_TASK_CHAT_MAX=120` is the sidebar **preview** `max_len`, not the input pre. `ascii_bounded` here rejected live Unicode specialize tasks in debug OXTs. CrossHair `DEAL_MAX_ORIGIN` / `DEAL_MAX_URL` are **32** (pytest **256**); `DEAL_MAX_HTML_CHUNK` is **16** (pytest **4096**; `feed()` and `strip_html_tags` slice to that bound so whole-string HTML / long `_append_response` chunks are not a pre; `_feed_chunk` keeps the bound). Range formatters use `DEAL_MAX_TOKEN` (not `CELL_REF=4`, which cannot fit `Sheet!A1`).

The pytest domain is the product (ZZZ, Calc max row, `CELL_REF=256`); the CrossHair domain is the short table in `deal_maxima(crosshair=True)`; `WRITERAGENT_CROSSHAIR=1` is set by the check-all runner, cover-all (including process-pool workers and `ProcessPoolExecutor` initializer), and `scripts/crosshair_stream.py run` (`make crosshair-check` / `make crosshair-cover`) before `deal_shim` import. Pytest / `make test` never set it. Do not grow `DEAL_MAX_COL_INDEX` / `DEAL_MAX_COL_LETTERS` out of sync: `parse_address("ZZZ1")` calls `column_to_index`, and a nested `PreContractError` is a CrossHair error. Nested inverse ensures (`format_address` → `parse_address`, `column_to_index` → `index_to_column`) are skipped under CrossHair via import-time `inverse_ensure`.
