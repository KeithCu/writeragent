...
- **`convert_model_to_dag` dep regex / `_` placeholder:** Check-all [33940151004](https://github.com/KeithCu/writeragent/actions/runs/33940151004) `PatternError: unterminated character set at position 0` on `convert_model_to_dag(ExcelWorkbookModel(scripts=[''], cells=[ExcelPyCell(..., deps=['_'], ...)], ..., source_path='\x00'), best_effort=True)`. **Not** the #599 NUL-scripts hole (`PreconditionFailed` / alphabet): CPython already fail-closes `resolve_dep('_')` as unrecognized; CrossHair's relib re-parses `_TABLE_ALL_RE`'s `\[#All\]` and `_RANGE_RE`'s `[^']` / `[A-Za-z_]` when a 1-char `_` (legal under `ascii_bounded`, prefix of `_xlfn.`) reaches `re.match`. Gate each regex on a cheap surface shape (`ANCHORARRAY(`, `[#All]`, trailing `#`, A1/range) so junk never hits `re.match`; CrossHair `_deal_convert_cell_ok` uses a closed A1-ish alphabet (no `_` / `[` / `\` / NUL). Pytest stays `ascii_bounded` so `_xlfn.ANCHORARRAY` / `Table[#All]` remain legal. Do **not** `# crosshair: off` `convert_model_to_dag`.
- **`convert_model_to_dag` → `_normalize_bindings` list pre:** Check-all [33954468213](https://github.com/KeithCu/writeragent/actions/runs/33954468213) `PreconditionFailed: type(resolved) is list` on `convert_model_to_dag(..., deps=['M'], formula_raw='\\x00', sheets=[SheetInfo('\\x01', ...)], best_effort=False)`. **Not** a non-list from `resolve_deps`: CPython already returns `[ResolvedDep(..., kind='unresolved', note="unrecognized dep 'M'")]`. deal names the first conjunct when a later one fails — CrossHair `_DEAL_NOTE_LEN=1` rejected that 20-char note. Align `_normalize_bindings` resolved-pre with `resolve_deps` `@deal.post` (`_deal_resolved_list_ok`: list of `ResolvedDep` + known kind); drop the leftover note/a1 length caps (the helper is `# crosshair: off`). `resolve_deps` is off with that post so convert can assume a list. Convert cell pre also `str_bounded`s `sheet` (resolve_dep `sheet_hint`).
