...
- **Scope**: `packages = ["plugin", "compute_service"]` with path **`exclude`** plus **`[[tool.mypy.overrides]]`** `ignore_errors = true` for **`plugin.contrib.*`**, **`plugin.lib.*`**, and **`plugin.tests.*`**. Plain `exclude` alone does not stop mypy from checking vendored trees when resolving the `plugin` package, so the overrides mirror ty’s “no contrib / no lib / no tests” intent. **`pint` / `pint.*`** use **`follow_imports = "skip"`** so mypy does not parse third-party internals (`python_version = "3.11"` cannot read pint 0.26 PEP 695 type-params; GHA 34520496629). Do **not** bump that mypy version to 3.12 to “fix” pint.
- **CI install**: PR CI and CrossHair deep use **`uv sync --frozen --dev`**. **`uv.lock` is tracked** (it used to be in `.gitignore`, so unfrozen `uv sync --dev` resolved unpinned extras from PyPI and pulled pint 0.26 the day it shipped — typecheck died before any UNO test; GHA 34520496629 / 34520753637). Dev extra pins **`pint>=0.25.3,<0.26`**.
...
- **Mode**: `typeCheckingMode = "all"` with unknown-type rules and project-idiom rules turned off so the gate stays useful. **On:** `reportMissingParameterType` and `reportMissingTypeArgument` (after leftover mop). Intentionally disabled (see comments in **`pyproject.toml`**):
  ...
  - **`reportUnannotatedClassAttribute`** stays off. Class-body annotations are in place for `plugin/calc`, `plugin/librepy`, `compute_service`, `plugin/mcp`, `plugin/acp`, `plugin/notebook`, `plugin/doc`, `plugin/draw`, `plugin/embeddings`, `plugin/ppt_master`, plugin toplevel, and `plugin/vision`. Remaining unannotated attrs are in `plugin/chatbot`, `plugin/writer`, `plugin/framework`, and `plugin/scripting`. Do not annotate Enum members to chase this rule, and do not silence it with `@final`.
  - **`reportUnannotatedClassAttribute`** stays off. `plugin/framework/` and `plugin/scripting/` instance attributes are annotated (dry-run of this rule is 0 there). Other trees still report hundreds of ordinary class attributes. Do not annotate Enum members to chase this rule.
