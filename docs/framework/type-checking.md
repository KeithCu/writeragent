...
- **Mode**: `typeCheckingMode = "all"` with unknown-type rules and project-idiom rules turned off so the gate stays useful. **On:** `reportMissingParameterType`, `reportMissingTypeArgument`, and `reportUnannotatedClassAttribute` (after leftover mop and instance-attr farms #864–#867). Enum members stay exempt; do not silence the class-attr rule with `@final`. Intentionally disabled (see comments in **`pyproject.toml`**):
