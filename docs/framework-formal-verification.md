...
make crosshair-check-all-deep   # same set; deep budget (200 iters / 30s per condition, no module wall)
...
- **Inputs**:
  ...
  - `deep`: `true` (default: 200 iterations; check-all 30s per condition / no module wall; cover-all no per-condition timeout) or `false` (regular: 25 iterations / 5s + 120s wall)
