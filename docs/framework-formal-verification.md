...
**Reference implementation:** [`plugin/scripting/payload_codec.py`](../plugin/scripting/payload_codec.py) — see [`docs/scripting-serialization-verification.md`](scripting-serialization-verification.md).
...
- **Nested callee pre after growth:** `sanitize_inline_py_code('dtype=float\x00')` (and escape/rebuild) used to `PreconditionFailed` on `_rewrite_token_calls`'s `str_bounded` because `dtype=float` → `dtype=np.float64` grows past CrossHair's `DEAL_MAX_SOURCE=16`. Call the unwrapped rewrite body from sanitize; keep `@deal` on the wrapper. Do not weaken the wrapper pre so CrossHair can enter the regex loop.
- **`format_data_binding_display` / `parse_data_binding_text`:** A1 / range tokens — `ascii_bounded` (still includes `\x1c` control whitespace). Unicode `str_bounded` + strip/split was hang-class (~1.36M lines) even when the check exited 0.
