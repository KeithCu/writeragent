...
- **Math (MathML) in HTML import**: When `apply_document_content` / `format_support` HTML import sees ``<math>...</math>``, content is split in [`html_math_segment.py`](plugin/modules/writer/html_math_segment.py); MathML is converted to StarMath via a hidden Math document in [`math_mml_convert.py`](plugin/modules/writer/math_mml_convert.py) and inserted as Writer formula objects in [`math_formula_insert.py`](plugin/modules/writer/math_formula_insert.py) (orchestrated from [`format_support.py`](plugin/modules/writer/format_support.py)). Broader design notes: [`docs/libreoffice-html-math-proposal.md`](docs/libreoffice-html-math-proposal.md), [`docs/libreoffice-html-math-dev-plan.md`](docs/libreoffice-html-math-dev-plan.md).
