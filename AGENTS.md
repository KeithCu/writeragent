...
- **Build & Dev**: `make build` (runs **`ty`** then **`ruff`** on **`plugin/`**, then bundle), `make deploy`. **`plugin/_manifest.py`** is gitignored; **`make ty`**, **`make check`** (ty only), **`make ruff`** (Ruff lint + flake8-type-checking **`TC`** rules; **`[tool.ruff]`** **`line-length`** **300**, **`[tool.ruff.format]`** **`skip-magic-trailing-comma`** **true** — collapsed calls when they fit the line — in **`pyproject.toml`**), **`make typecheck`** (ty + mypy + pyright), and **`make test`** all use **`make manifest`** where applicable so clean checkouts get a generated manifest before type-check. If the file is still absent, [`plugin/framework/module_loader.py`](plugin/framework/module_loader.py) `load_manifest()` raises **`RuntimeError`** (no silent empty module list). **External tools**: `make fix-uno` to link system UNO into `.venv`. **Typecheckers**: **`make check`** → **`ty`** only; **`make build`** → **`ty`** + **`ruff`**; **`make typecheck`** → **`ty` + mypy + pyright**; **`make test`** → typecheck, then **`bandit`** on **`plugin/`** (excludes **`plugin/contrib`**, **`plugin/lib`**, and **`plugin/tests`**, see **`[tool.bandit]`** in **`pyproject.toml`**), then pytest + LO tests; **`make release`** runs **`make test`** first, then **`release-build`** (includes **`openrouter-catalog`** → [`registry/openrouter_models.json`](registry/openrouter_models.json), not bundled in the OXT, plus [`plugin/framework/default_models.py`](plugin/framework/default_models.py), then bundle). 
  ...
  - **Leaked chat-template tokens**: `LlmClient.request_with_tools` removes ``<|alphanumeric_underscore|>``-style control tokens from assistant `content` after every response (local / Harmony-style models sometimes leak them; if that text is sent back on the next turn, the server can return HTTP 500 "Failed to parse input"). On strip, **info** logs model + lengths; **debug** logs full `repr` of original and cleaned. Each `request_with_tools` call also **debug**-logs model, `stream`, message count, and tool-def count. On non-200 HTTP from the provider, **error** logs the outgoing `messages` array (via `redact_sensitive_payload_for_log`) after the error body. See `strip_leaked_chat_template_control_tokens` in [`plugin/modules/http/client.py`](plugin/modules/http/client.py).
...
- **Query keyboard**: Sidebar multiline query field: **Enter** runs the same primary action as **Send** when the Send button is enabled and **Settings → Doc → Enter key sends sidebar chat message** is on (`doc.chat_enter_key_sends_message`, default true); **Shift+Enter** inserts a newline. Key classification [`query_enter_triggers_primary_send`](plugin/modules/chatbot/panel.py); [`QueryKeyListener`](plugin/modules/chatbot/panel.py) + [`panel_wiring.py`](plugin/modules/chatbot/panel_wiring.py); schema [`plugin/modules/doc/module.yaml`](plugin/modules/doc/module.yaml).
- **Send / document model**: [`SendButtonListener._get_document_model`](plugin/modules/chatbot/panel.py) resolves the document with **`frame.getController().getModel()`** first (same window as the sidebar), then falls back to **`get_active_document`** (`Desktop.getCurrentComponent()`). Using only the desktop “current component” fails when focus is in the sidebar query field. If resolution still fails, a single **`log.error`** records frame vs desktop probes (no document body). Unit coverage: [`plugin/tests/test_send_button_listener_document_model.py`](plugin/tests/test_send_button_listener_document_model.py).
...
**Smolagents HTTP (librarian / specialized delegation)**: [`WriterAgentSmolModel`](plugin/framework/smol_model.py) is the **only** code path that should call `LlmClient.request_with_tools` for vendored smolagents. It passes smolagents' generated OpenAI tool schemas (`completion_kwargs["tools"]`) on the wire while tool definitions also appear in the smol system prompt via `__TOOLS_LIST__`. This preserves the known-good LocalAI request shape while still gaining LlmClient's stripping/shims/pacing. Do not add a second HTTP entry point or a user config toggle for smol wire tools; if this breaks another backend, handle it deliberately in `WriterAgentSmolModel` with tests. **Shared smol glue**: [`smol_tool_adapter.py`](plugin/framework/smol_tool_adapter.py) (`to_smol_inputs`, `SmolToolAdapter` with `inputs_style` `librarian` vs `specialized`, and `safe` / `main_thread_sync` for `execute` vs `execute_safe` + main thread); [`smol_agent_factory.py`](plugin/framework/smol_agent_factory.py) (`build_toolcalling_agent` — single construction site for `LlmClient` + `WriterAgentSmolModel` + `ToolCallingAgent` with `chat_max_tokens` / `chat_max_tool_rounds`). Librarian and specialized delegation import these; do not duplicate ad-hoc `ToolCallingAgent` setup. **Design (minimal branching, reuse below the wire policy)**: [docs/smol-main-chat-tool-architecture.md](docs/smol-main-chat-tool-architecture.md).
...
- **Direct image** (`chat_direct_image`): runs `generate_image` on the tool path; `generate_image` is async (`ToolWriterImageBase` in [`images.py`](plugin/modules/writer/images.py)); sub-agent / delegation uses [`SmolToolAdapter`](plugin/framework/smol_tool_adapter.py) in specialized paths ([`specialized.py`](plugin/modules/writer/specialized.py) / [`specialized_base.py`](plugin/framework/specialized_base.py)).
- **Specialized Delegation**: Gateway tools (`DelegateToSpecializedWriter`, `DelegateToSpecializedCalc`, `DelegateToSpecializedDraw`) inherit from `DelegateToSpecializedBase` in [`specialized_base.py`](plugin/framework/specialized_base.py). They spin up a sub-agent using `smolagents` for complex tasks in specific domains (e.g., images, styles, pivot tables). Draw now supports the full sub-agent path via this shared base. **`footnotes_insert`** accepts **`insert_after_text`** (optional): document substring to find so the footnote anchor is placed after that match when the sub-agent cannot rely on the view cursor. For **`domain=shapes`**, the sub-agent instructions include automatic canvas context from [`specialized_shapes_context.py`](plugin/framework/specialized_shapes_context.py): Writer (page style + margins), Calc (active sheet draw page + sheet index), Draw, and Impress (active slide + slide index); coordinates are described in **1/100 mm**. The main chat agent is unchanged and can use existing tools if it needs layout data.
...
**Librarian + models**: Onboarding uses vendored smolagents [`ToolCallingAgent`](plugin/contrib/smolagents/agents.py) with plain-text ReAct `Action:` examples ([`toolcalling_agent_prompts.py`](plugin/contrib/smolagents/toolcalling_agent_prompts.py)). [`WriterAgentSmolModel`](plugin/framework/smol_model.py) calls `LlmClient.request_with_tools(..., tools=completion_kwargs.get("tools"), prepend_dev_build_system_prefix=False)`: tool schemas populate both the smol system prompt (`__TOOLS_LIST__`) and the OpenAI-compatible HTTP body. Smol parses tool calls from assistant text via `parse_tool_calls` / `parse_json_blob`, or native `tool_calls` when returned.
...
**Impress headers/footers** (`get_headers_footers` / `set_headers_footers`, Presentation only): when `is_master_page` is true, `page_index` is a **slide** index; the tool resolves that slide’s **`MasterPage`** (do not use `getMasterPages().getByIndex(page_index)` — collection order differs from the slide’s assigned master) — [`plugin/modules/draw/headers_footers.py`](plugin/modules/draw/headers_footers.py). Boolean args accept JSON-style strings. **Impress slide masters** omit `FooterText` / `IsFooterVisible` etc. from the master’s `XPropertySet` in LibreOffice; for `PresentationDocument` + `is_master_page`, the tool reads/writes **`HeaderShape` / `FooterShape` / `DateTimeShape` / `SlideNumberShape`** placeholder text and `Visible` instead of page properties (slides still use `getPropertyValue` / `setPropertyValue`).
...
**Specialized Calc Toolsets**: Operations like managing cell comments, conditional formatting, standard sheet filter (AutoFilter-style), sheet management, image management, **pivot tables (DataPilot)**, **form controls** on the active sheet, and **track changes** (`track_changes_*` tools, same names as Writer) live in specialized domains (`comments`, `conditional_formatting`, `sheet_filter`, `sheets`, `images`, `pivot_tables`, `forms`, `tracking`). Use `delegate_to_specialized_calc_toolset(domain="tracking")` for record/stop/list/show/accept/reject tracked changes. Listing or per-index accept/reject may return empty or require `getRedlines` on the model depending on LibreOffice version. **`track_changes_show` on Calc**: not implemented from the extension (returns a short notice; use LibreOffice menus to show markup). Writer-only **annotation** comment tools (`track_changes_comment_*`) stay Text-document-only. These tools inherit from `ToolCalcSpecialBase` (and shared tracking classes also from `ToolWriterSpecialBase`) and are filtered out of the core tool list. Conditional formatting (classic `TableConditionalFormat` / `add_conditional_format`, list, remove): [`plugin/modules/calc/conditional.py`](plugin/modules/calc/conditional.py); UNO background and roadmap: [docs/calc-conditional-formatting.md](docs/calc-conditional-formatting.md). Sheet filter: [`plugin/modules/calc/sheet_filter.py`](plugin/modules/calc/sheet_filter.py); [docs/calc-sheet-filter.md](docs/calc-sheet-filter.md) (includes future scope / roadmap in §5). Pivot tools: [`plugin/modules/calc/pivot.py`](plugin/modules/calc/pivot.py) (`ToolCalcPivotBase`). Form tools are implemented in [`plugin/modules/writer/forms.py`](plugin/modules/writer/forms.py): [`ToolWriterFormBase`](plugin/modules/writer/base.py) subclasses `ToolWriterSpecialBase`, `ToolCalcSpecialBase`, and `ToolDrawFormBase` (single `specialized_domain`), with union `uno_services` on each concrete tool. These tools support Writer, Calc, Draw, and Impress documents.
