...
**Smolagents HTTP (librarian / specialized delegation)**: [`WriterAgentSmolModel`](plugin/framework/smol_model.py) is the **only** code path that should call `LlmClient.request_with_tools` for vendored smolagents. It always passes **`tools=None`** on the wire (module constant `SMOL_HTTP_TOOLS_ON_WIRE`); tool definitions still appear in the smol system prompt via `__TOOLS_LIST__`. **Keeps smolagents' text-based tool parsing while gaining LlmClient's stripping/shims/pacing.** Do not add a second HTTP entry point or forward `completion_kwargs["tools"]` to the API body—some local servers return HTTP 500 when `tools` activates constrained parsing. **Shared smol glue**: [`smol_tool_adapter.py`](plugin/framework/smol_tool_adapter.py) (`to_smol_inputs`, `SmolToolAdapter` with `inputs_style` `librarian` vs `specialized`, and `safe` / `main_thread_sync` for `execute` vs `execute_safe` + main thread); [`smol_agent_factory.py`](plugin/framework/smol_agent_factory.py) (`build_toolcalling_agent` — single construction site for `LlmClient` + `WriterAgentSmolModel` + `ToolCallingAgent` with `chat_max_tokens` / `chat_max_tool_rounds`). Librarian and specialized delegation import these; do not duplicate ad-hoc `ToolCallingAgent` setup.
...
Fallback when API returns text without `tool_calls`: [`plugin/modules/http/client.py`](plugin/modules/http/client.py) → `get_parser_for_model` → [`plugin/contrib/tool_call_parsers/`](plugin/contrib/tool_call_parsers/). **Smolagents** uses the same `LlmClient.request_with_tools` post-processing (token stripping, fallback parsers) via [`WriterAgentSmolModel`](plugin/framework/smol_model.py) only, with **`tools=None`** on the wire by policy (`SMOL_HTTP_TOOLS_ON_WIRE`); see §4 **Smolagents HTTP**. Hermes backend: leading `/` messages forwarded on ACP without `[DOCUMENT CONTENT]` wrapping. Upstream lineage: [hermes-agent](https://github.com/NousResearch/hermes-agent). Chat **Agent backends → Hermes** spawns only `hermes acp`; if LibreOffice’s environment has a narrower `PATH` than your shell, set **Path / URL** to the full `hermes` executable and leave **Extra arguments** empty for the default `acp`.
