...
- **Self-Learning (Experimental)**: Tools for agent-curated persistent memory (`MEMORY.md`, `USER.md`) and procedural skills (`SKILL.md`) have been ported from hermes-agent to `plugin/modules/chatbot/tools/`. They are currently commented out in `plugin/modules/chatbot/__init__.py`, `constants.py`, and `document.py` to keep them hidden from the active API while available for manual experimentation.
...
│   │   ├── state.py         # BaseState, FsmTransition, shared pure FSM contracts
...
- **Sidebar panel**: WriterAgent deck in Writer's or Calc's right sidebar; panel has Response area, Ask field, Send button, Stop button, and Clear button. **Theme Matching**: The sidebar automatically adapts to LibreOffice's current color scheme (including Dark Mode) using the native VCL theming system. When the user changes Settings (e.g. model or additional instructions), the sidebar is notified via **config-change listeners** in `plugin/framework/config.py` (`add_config_listener`, `notify_config_changed`); the panel refreshes its model and prompt selectors from config so they stay in sync. Listeners use weakref so panels can be GC'd without unregistering.
  ...
  - **State machines (shared FSM contract)**: `plugin/framework/state.py` defines `BaseState` (frozen dataclass root), an `Effect` typing protocol, `FsmTransition[StateT]` (successor state + effect list), and `unpack_transition()` for legacy tuple shapes. Each domain uses a **pure** module-level `next_state(state, event) -> FsmTransition[...]` with **no** UNO, I/O, logging, or `EventBus` inside the transition. Side effects are only described as effect values and run synchronously in interpreters (panel, mixins, MCP handler). Domain modules: `plugin/modules/chatbot/send_state.py` (Send/Stop/Record; typed send effects such as `StartSendEffect` / `StartRecordingEffect` are **panel-level** dataclasses—distinct from same-named classes in `audio_recorder_state.py` used inside `AudioRecorder`), `audio_recorder_state.py`, `state_machine.py` (audio / direct image / agent / web research paths), `tool_loop_state.py` (multi-round tool calling), `plugin/modules/http/mcp_state.py` (MCP `tools/call`). **`sidebar_state.py`** composes `SidebarCompositeState` (`send`, optional `tool_loop`, mirrored `audio`), `SidebarEvent` (`kind` + `payload`), and pure `sidebar_next_state()`; tool-loop events with no active session return `LogSidebarEffect` only. `SendButtonListener` holds `sidebar_state`; `state` is a property alias for `sidebar_state.send`. `ToolCallingMixin` stores the tool loop in `sidebar_state.tool_loop` via a `_sm_state` property; the slice is cleared in a `finally` block in `_start_tool_calling_async` (only after the drain loop returns or on setup errors). `sync_audio_slice()` copies `audio_recorder.state` into the composite after recording-related effects. Use **`EventBus`** only for loose cross-module notifications (e.g. config, `mcp:request`); do not use it as the FSM driver.
- **Implementation**: `plugin/modules/chatbot/panel_factory.py` (ChatPanelFactory, ChatPanelElement, ChatToolPanel); `ContainerWindowProvider` + `ChatPanelDialog.xdl`; `setVisible(True)` required after `createContainerWindow()`. The sidebar layout and resize behavior (anchoring controls and stretching the response area) are handled by `_PanelResizeListener` in `plugin/modules/chatbot/panel_resize.py`, which is wired from `plugin/modules/chatbot/panel_wiring.py`.
...
- **OpenRouter STT Cleanup**: OpenRouter's STT model list was streamlined to keep `google/gemini-3.1-flash-lite-preview` as the primary cross-platform audio-native option.
    - **Writer**: `com.sun.star.text.TextDocument`. Writer tools are registered in `plugin/modules/writer/__init__.py` and expose `get_document_content`, `apply_document_content` (with **`old_content`** for StrReplace-style section replacement; no positions needed), `search_in_document` (in `plugin/modules/writer/search.py`; use `return_offsets` for character positions) + `get_document_tree`, `get_document_stats`, `list_styles`, `get_style_info` + `generate_image` (create or edit with `source_image='selection'`). Paragraph-oriented tools in `plugin/modules/writer/content.py` (`read_paragraphs`, `insert_at_paragraph`, `modify_paragraph`, `delete_paragraph`, `duplicate_paragraph`, `clone_heading_block`, `insert_paragraphs_batch`) use `ToolBaseDummy` and are **not** registered (change base to `ToolBase` to re-expose). **Table tools** (`list_tables`, `read_table`, `write_table_cells`, etc.) are defined in `plugin/modules/writer/tables.py` but kept **fully commented out** and not registered; to re-enable, uncomment those classes, wire them in `__init__.py`, and set `WRITER_TABLE_TOOLS_DISABLED = False` in `plugin/tests/smoke_writer_tools.py`. Additional advanced tools (comments, text frames, track-changes, navigation, sections/bookmarks, etc.) use `ToolBase` or `ToolBaseDummy` as appropriate. The former `replace_in_document` tool was removed; use `apply_document_content(old_content=..., content=...)` instead.
...
- **Web search checkbox**: The chat sidebar includes a **Web search** checkbox (`web_search_check`) below the Send/Stop/Clear buttons. Toggling it switches between document chat and web-only mode and updates the response-area greeting (`get_greeting_for_document` vs `DEFAULT_RESEARCH_GREETING` via `plugin.framework.i18n._`). `ResearchChatToggledListener` in `plugin/modules/chatbot/panel_factory.py` imports `_` inside `on_item_state_changed`; the path-climbing loop at the top of that file must not use `for _ in ...` (it would bind module-level `_` and break `_(...)`).
  ...
  - **Prompt before search (Settings)**: Config `chatbot.prompt_for_web_research` requires confirmation before each `web_search` tool call when an `approval_callback` is present. The UI maps **Send → Accept**, **Stop → Change**, **Clear → Reject**: **Accept** runs the query as proposed; **Change** opens a modal multiline dialog (`show_web_search_query_edit_dialog` in `plugin/framework/dialogs.py`) to edit the DuckDuckGo query, then continues with the edited text (Cancel returns to the approval row); **Reject** aborts like Stop. The approval handshake returns `(proceed, query_override)` to `web_research.py`, which applies overrides via `_apply_web_search_query_override` (coerces JSON-string or non-dict `ToolCall.arguments` to a dict so DuckDuckGo always sees the edited query). **Change** also appends a `[Web search]` chat block for the edited query (`panel.py`). Rejecting no longer triggers `RuntimeError: generator ignored GeneratorExit`: vendored `MultiStepAgent._run_stream` in `plugin/contrib/smolagents/agents.py` skips the per-step `yield action_step` in `finally` when the generator is closing (`GeneratorExit`). The status line shows a short “waiting” hint. **Chat copy for research** comes from the sub-agent’s internal DuckDuckGo steps only (`web_search_engine_step_chat_text` in `web_research_chat.py`): the **first** distinct `web_search` preview uses the full `[Web search]` block unless it would duplicate the outer `query` passed into `web_research` (then that preview is skipped without advancing the step index); **further** internal searches use `[Additional web search]`. There is no separate `[Web research] / Research request:` banner in the response area; the tool loop uses the same generic `[Running tool: web_research...]` line as other tools in the stream. The approval path still uses `web_research_engine_chat_block` (step 0 shape) for each inline prompt. The tool result line uses a short completion message (e.g. translated “Web research completed.”). Non-approval path uses `ToolContext.chat_append_callback`. The sidebar web-research path and the main tool-calling loop pass approval via `ToolContext` (queue + main-thread handler). `WebResearchTool.is_async()` returns true so execution stays off the UI thread and the approval queue cannot deadlock the drain loop. **ACP Backend Approval**: External ACP processes (like Mistral Vibe) that send `session/request_permission` notifications rely on `submit_approval` in `ACPBackend` which replies with a JSON-RPC response; the UI uses a `QUERYBOX` (4) message box to ensure Yes/No buttons are displayed and correctly mapped to approval.
...
- **Writer cursor handling** (`plugin/framework/image_tools.py`): When inserting inline Writer images, the code converts the controller `ViewCursor` into a document `TextCursor` before calling `Text.insertTextContent`, avoiding UNO errors like `text interface and cursor not related` / `no text selection`.
...
- **Chat sidebar** (`WriterAgentDialogs/ChatPanelDialog.xdl`, `plugin/modules/chatbot/panel_factory.py`): **AI Model** combobox (text model → `text_model`, `model_lru`) and **Image model (same endpoint as chat)** combobox (→ `image_model`, `image_model_lru`). **"Use Image model"** checkbox (config `chat_direct_image`): when checked, the current message is sent directly to the image pipeline (AI Horde or image model per Settings) for Writer, Calc, and Draw — no chat model round-trip. Orthogonal to which tools are given to the LLM; uses `document_tools.execute_tool("generate_image", ...)` for all doc types. No additional-instructions control in the sidebar; extra instructions come from config only when building the system prompt.
  - **Resize behavior & GTK width guard (March 2026)**: Sidebar layout on resize is handled by `_PanelResizeListener` in `plugin/modules/chatbot/panel_resize.py`, which snapshots the XDL-defined positions/sizes once and then keeps buttons/fields anchored while stretching the response/query/model combobox areas. `ChatToolPanel.getHeightForWidth()` uses **`eff_w = parent_w`** when parent and deck hint are within ~80px (fluid controls fill the sidebar); when **`parent_w > deck_hint + 80`** (intrinsic inflation vs real allocation), **`eff_w = min(parent_w, deck_w)`**. `_relayout` applies the same rule. **`_last_deck_w`** stores the latest deck hint for that sync. **Send** keeps a fixed pixel width so Record/Send toggles do not widen the panel. Fluid controls use per-control minimum widths (model comboboxes ≥120) **without exceeding available horizontal space**, so the dropdown button is not clipped on narrow sidebars. Debug logs: `getHeightForWidth deck_hint=... eff_W=...`, `_relayout: sync root ... (parent=... deck=...)`, `_relayout fluid widths=...`, `_capture_initial ctrl_widths=...`. Toggling **Use Image model** runs a relayout so the visible model row keeps correct widths. `ChatPanelDialog.xdl` baseline panel width is 180 Map AppFont units to align with `getMinimalWidth()`.
...
- **Navigation / paragraph tools**: `get_document_outline` and `get_heading_content` are not registered. The paragraph batch tools in `content.py` listed above are present in code but disabled via `ToolBaseDummy` until re-enabled.
...
- **Settings Dialog Integration**: AI and service configurations are unified in the `WriterAgent -> Settings` dialog. The Settings dialog tabs for service configuration (e.g., Http, Launcher, Chatbot, Agent Backend) are auto-generated from `module.yaml` configuration schemas by `scripts/generate_manifest.py`. Writer has no config so no Writer tab; Tunnel is skipped in the UI (see Section 3). **`core`** (internal-only keys such as `log_level`) is skipped for tab generation in `scripts/manifest_registry.py`; `plugin/framework/legacy_ui.py` `settings_box` must skip **`core`** in the same loop—otherwise `dlg.getControl("btn_tab_core")` throws and Settings never opens.
...
**Gettext / UI languages**: `make build` compiles `plugin/locales/*/LC_MESSAGES/writeragent.mo` and the OXT bundles **`plugin/locales/`** so runtime `gettext` can load translations for LibreOffice’s `ooLocale`. `translate_dialog()` walks the **full** `XControlContainer` tree (not only top-level controls), so labels inside bulletinboards (e.g. Settings) are translated. Menus/sidebar titles in `.xcu` files remain English until given `xml:lang` entries separately. **String extraction**: `make extract-strings` runs XDL stub generation, `xgettext` on `plugin/**/*.py`, then `scripts/merge_module_yaml_into_pot.py` to append `module.yaml` strings to `writeragent.pot` as plain translatable strings so the generic `translate_dialog` function can match them. After extraction, refresh each locale with `msgmerge` against the updated POT (see `plugin/locales/README.md`). **`make auto-translate`** runs `extract-strings` first, then `scripts/translate_missing.py` (preview, then optional `--execute` if `OPENROUTER_API_KEY` is set). The status table compares each locale to `writeragent.pot` (pending vs template); `translate_missing.py` batches API calls in one thread pool across **all** languages (`-j` = max concurrent requests). Each string sent to the model has leading/trailing whitespace peeled off and reapplied after translation so the model only translates the trimmed core.
...
- **Icons**: `assets/` includes `running_16.png`, `running_26.png`, `starting_16.png`, `starting_26.png`, `stopped_16.png`, `stopped_26.png` (from libreoffice-mcp-extension). Canonical extension default logo is `extension/assets/logo.png` (`description.xml`); sidebar deck icon is `extension/assets/logo_32.png`.
...
- **DO NOT shadow global modules**: Be extremely careful with `import logging` inside functions if `logging` is also imported at the top level. This can cause `UnboundLocalError` when the function tries to use the global `logging` but sees a local "not yet initialized" name instead. Same for **`log`**: do not use `from plugin.framework.logging import log` inside a function in a module that already defines `log = logging.getLogger(__name__)` at module level—that binds `log` as a local for the *whole* function and breaks earlier `log.debug(...)` calls (`UnboundLocalError`).
