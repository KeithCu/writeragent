...
**Smolagents HTTP (librarian / specialized delegation)**: [`WriterAgentSmolModel`](plugin/framework/smol_model.py) is the **only** code path that should call `LlmClient.request_with_tools` for vendored smolagents. It passes **`tools=None`** on the wire; tool definitions still appear in the smol system prompt via `__TOOLS_LIST__`. **Keeps smolagents' text-based tool parsing while gaining LlmClient's stripping/shims/pacing.** Do not add a second HTTP entry point or forward `completion_kwargs["tools"]` to the API body in shipped builds (many local servers return HTTP 500 when `tools` activates constrained parsing). **Shared smol glue**: [`smol_tool_adapter.py`](plugin/framework/smol_tool_adapter.py) (`to_smol_inputs`, `SmolToolAdapter` with `inputs_style` `librarian` vs `specialized`, and `safe` / `main_thread_sync` for `execute` vs `execute_safe` + main thread); [`smol_agent_factory.py`](plugin/framework/smol_agent_factory.py) (`build_toolcalling_agent` — single construction site for `LlmClient` + `WriterAgentSmolModel` + `ToolCallingAgent` with `chat_max_tokens` / `chat_max_tool_rounds`). Librarian and specialized delegation import these; do not duplicate ad-hoc `ToolCallingAgent` setup. **Design (minimal branching, reuse below the wire policy)**: [docs/smol-main-chat-tool-architecture.md](docs/smol-main-chat-tool-architecture.md).
...
**Librarian + models**: Onboarding uses vendored smolagents [`ToolCallingAgent`](plugin/contrib/smolagents/agents.py) with plain-text ReAct `Action:` examples ([`toolcalling_agent_prompts.py`](plugin/contrib/smolagents/toolcalling_agent_prompts.py)). [`WriterAgentSmolModel`](plugin/framework/smol_model.py) calls `LlmClient.request_with_tools(..., tools=None, prepend_dev_build_system_prefix=False)`: tool schemas still populate the smol system prompt (`__TOOLS_LIST__`); OpenAI `tools` are not sent in the HTTP body by default (local servers, e.g. llama.cpp, may 500); smol parses tool calls from assistant text via `parse_tool_calls` / `parse_json_blob`, or native `tool_calls` when returned.
