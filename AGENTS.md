...
**Librarian + models**: Onboarding uses vendored smolagents [`ToolCallingAgent`](plugin/contrib/smolagents/agents.py) with plain-text ReAct `Action:` examples ([`toolcalling_agent_prompts.py`](plugin/contrib/smolagents/toolcalling_agent_prompts.py)). [`WriterAgentSmolModel`](plugin/framework/smol_model.py) calls `LlmClient.request_with_tools(..., prepend_dev_build_system_prefix=False)` so smol system prompts are not prefixed with the optional dev-build paragraph (release bundles omit that paragraph anyway).
