...
**Gettext / UI languages**: `make build` compiles `plugin/locales/*/LC_MESSAGES/writeragent.mo` and the OXT bundles **`plugin/locales/`** so runtime `gettext` can load translations for LibreOffice’s `ooLocale`. `translate_dialog()` walks the **full** `XControlContainer` tree (not only top-level controls), so labels inside bulletinboards (e.g. Settings) are translated. Menus/sidebar titles in `.xcu` files remain English until given `xml:lang` entries separately. **String extraction**: `make extract-strings` runs XDL stub generation, `xgettext` on `plugin/**/*.py`, then `scripts/merge_module_yaml_into_pot.py` to append `module.yaml` strings to `writeragent.pot` (with `msgctxt`); then refresh each locale with `msgmerge` against the updated POT (see `plugin/locales/README.md`).
