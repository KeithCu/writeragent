...
- **Math in HTML import (MathML + TeX)**: When `apply_document_content` / `format_support` HTML import sees ``<math>...</math>`` and/or TeX delimiters (`$...$`, `$$...$$`, `\(...\)`, `\[...\]`), content is split in [`html_math_segment.py`](plugin/modules/writer/html_math_segment.py). **MathML** is converted to StarMath via a hidden Math document in [`math_mml_convert.py`](plugin/modules/writer/math_mml_convert.py); **TeX** is converted with vendored `latex2mathml` then the same MathML→StarMath path (`convert_latex_to_starmath`). Formulas are inserted in [`math_formula_insert.py`](plugin/modules/writer/math_formula_insert.py) (orchestrated from [`format_support.py`](plugin/modules/writer/format_support.py)). Stream order prefers MathML when both appear (earlier island wins). Broader design notes: [`docs/libreoffice-html-math-proposal.md`](docs/libreoffice-html-math-proposal.md), [`docs/libreoffice-html-math-dev-plan.md`](docs/libreoffice-html-math-dev-plan.md).
