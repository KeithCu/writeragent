...
**Librarian + models**: Onboarding uses vendored smolagents [`ToolCallingAgent`](plugin/contrib/smolagents/agents.py) with plain-text ReAct `Action:` examples ([`toolcalling_agent_prompts.py`](plugin/contrib/smolagents/toolcalling_agent_prompts.py)). Some provider routes for **GPT-OSS / Harmony-style** models reject those prompts (`HTTP 500`, “Failed to parse input”, `<|channel|>` in the error body). Prefer a standard OpenAI-style tool model for Librarian; [`WriterAgentSmolModel`](plugin/framework/smol_model.py) calls `LlmClient.request_with_tools(..., prepend_dev_build_system_prefix=False)` so the sidebar dev-build prefix does not inflate smol system prompts. User-facing hint when this pattern is detected: [`user_message_if_provider_harmony_tool_parse_failure`](plugin/framework/errors.py) from [`librarian.py`](plugin/modules/chatbot/librarian.py).
