...
- **Settings Dialog Integration**: AI and service configurations are unified in the `LocalWriter -> Settings` dialog. The build system supports a `LOCALWRITER_ENABLE_OPTIONS` flag (default `0`) which disables the legacy LibreOffice `Tools -> Options` generation. The Settings dialog tabs for service configuration (e.g., Core, Writer, Calc, Chatbot, Tunnel) are auto-generated from `module.yaml` configuration schemas by `scripts/generate_manifest.py`.
