...
- **Math in HTML import (TeX + MathML)**: When `apply_document_content` / `format_support` HTML import sees TeX delimiters (`$...$`, `$$...$$`, `\(...\)`, `\[...\]`) and/or `<math>...</math>`, content is split in [`html_math_segment.py`](plugin/modules/writer/html_math_segment.py). To prevent LaTeX corruption, literal `\\n`/`\\t` expansion is pushed down to the specific `html` segments in [`format_support.py`](plugin/modules/writer/format_support.py), ensuring that `tex` and `mathml` chunks are bypassed and their literal backslashes preserved. **TeX is the recommended input** — briefer and faster for agents to produce. Both paths produce editable LibreOffice Math objects. Formulas are inserted in [`math_formula_insert.py`](plugin/modules/writer/math_formula_insert.py). Broader design notes: [`docs/libreoffice-html-math-proposal.md`](docs/libreoffice-html-math-proposal.md), [`docs/libreoffice-html-math-dev-plan.md`](docs/libreoffice-html-math-dev-plan.md). Regression tests for this preservation logic live in [`plugin/tests/test_writer_math_preservation.py`](plugin/tests/test_writer_math_preservation.py).
