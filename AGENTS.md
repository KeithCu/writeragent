...
- **Native AI grammar (Linguistic2)**: [`ai_grammar_proofreader.py`](plugin/modules/writer/ai_grammar_proofreader.py) implements `XProofreader`; [`LinguisticWriterAgentGrammar.xcu`](extension/registry/org/openoffice/Office/LinguisticWriterAgentGrammar.xcu) registers it under `GrammarCheckers` with `Locales` **`en-US en-GB`** (one `oor:string-list` value, Lightproof-style). **Default builds omit this XCU** because re-bundling it still caused native LibreOffice crashes on some builds; opt in only for controlled tests with `WRITERAGENT_ENABLE_LINGUISTIC_GRAMMAR_XCU=1 make manifest` / `make build`. `WRITERAGENT_LINGUISTIC_GRAMMAR_STUB=1 make build` swaps the manifest to the no-op [`ai_grammar_proofreader_stub.py`](plugin/modules/writer/ai_grammar_proofreader_stub.py) and forces the XCU on for native-registration isolation. Keep `ai_grammar_proofreader.py` top-level imports minimal (stdlib + `unohelper` + UNO interfaces); linguistic enumeration imports this module before real proofreading starts. Doc-tab `doc.grammar_proofreader_enabled` gates LLM work if the proofreader is registered; startup does not call `setConfiguredServices`. Parity: `GRAMMAR_REGISTRY_LOCALE_TAGS` in [`grammar_proofread_engine.py`](plugin/modules/writer/grammar_proofread_engine.py) and [`plugin/tests/test_grammar_linguistic_xcu.py`](plugin/tests/test_grammar_linguistic_xcu.py).
