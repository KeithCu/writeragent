...
- **`plugin/modules/agent_backend/`**: Backend abstraction and registry; adapters for **Built-in**, **Aider**, **Hermes** (Aider/Hermes are stubs that report "not yet implemented"). Contract: `send(queue, user_message, document_context, document_url, ...)` pushes events (`chunk`, `thinking`, `status`, `stream_done`, `error`, `approval_required`) into the same queue the sidebar drain loop consumes.
- **Sidebar**: When Settings "Use external agent backend" is enabled and backend is Aider or Hermes, `_do_send` branches to `_do_send_via_agent_backend` (no fallback to built-in on failure). Stop button calls `adapter.stop()`. Backend indicator label in chat panel shows "Aider" or "Hermes" when enabled.
- **Settings**: Config keys `agent_backend.enabled`, `agent_backend.backend_id` (builtin | aider | hermes), and per-backend `agent_backend.aider_path`, `agent_backend.aider_args`, `agent_backend.hermes_path`, `agent_backend.hermes_args`. Auto-generated Settings tab from `plugin/modules/agent_backend/module.yaml`.
