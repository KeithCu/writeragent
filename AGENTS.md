...
- **Math in HTML import (TeX + MathML)**: When `apply_document_content` / `format_support` HTML import sees TeX delimiters (`$...$`, `$$...$$`, `\(...\)`, `\[...\]`) and/or `<math>...</math>`, content is split in [`html_math_segment.py`](plugin/modules/writer/html_math_segment.py). **TeX is the recommended input** — briefer and faster for agents to produce. **MathML** is also supported (pass it when you already have it). Both paths produce editable LibreOffice Math objects: TeX via `convert_latex_to_starmath` in [`math_mml_convert.py`](plugin/modules/writer/math_mml_convert.py); MathML via a hidden Math document in the same module. Formulas are inserted in [`math_formula_insert.py`](plugin/modules/writer/math_formula_insert.py) (orchestrated from [`format_support.py`](plugin/modules/writer/format_support.py)). When both appear in a fragment, whichever island starts first wins. Broader design notes: [`docs/libreoffice-html-math-proposal.md`](docs/libreoffice-html-math-proposal.md), [`docs/libreoffice-html-math-dev-plan.md`](docs/libreoffice-html-math-dev-plan.md).
...
- **Native AI grammar (Linguistic2)**: [`ai_grammar_proofreader.py`](plugin/modules/writer/ai_grammar_proofreader.py) implements `XProofreader`; [`LinguisticWriterAgentGrammar.xcu`](extension/registry/org/openoffice/Office/LinguisticWriterAgentGrammar.xcu) registers it under `GrammarCheckers` with `Locales` **`en-US en-GB`** (one `oor:string-list` value, Lightproof-style). **Default `make manifest` / `make build` always include** that XCU and the Python UNO module in [`scripts/manifest_registry.py`](scripts/manifest_registry.py). **Critical**: the proofreader class `__init__(self, ctx, *args)` must accept extra UNO constructor args — LibreOffice instantiates services via `createInstanceWithArgumentsAndContext`; omitting `*args` caused native `RuntimeException` / abort when opening Writing Aids. [`ai_grammar_proofreader_stub.py`](plugin/modules/writer/ai_grammar_proofreader_stub.py) remains for manual bisect only (not wired by manifest). Keep `ai_grammar_proofreader.py` top-level imports minimal (stdlib + `unohelper` + UNO interfaces); linguistic enumeration imports this module before real proofreading starts. Doc-tab `doc.grammar_proofreader_enabled` gates LLM work; optional `doc.grammar_proofreader_pause_during_agent` skips grammar worker requests while sidebar chat/agent sessions are active. On the **`nStartOfSentencePosition == 0`** pass, the proofread slice is **`aText[0:min(len(aText), doc.grammar_proofreader_max_chars)]`** (Lightproof-style batching; see `_finalize_proofreading_sentence_positions` and in-repo [`lightproof/Lightproof.py`](lightproof/Lightproof.py)), not Writer’s growing suggested end alone—so single-letter prefixes are not sent to the LLM. **In-process proofread cache** ([`grammar_proofread_engine.py`](plugin/modules/writer/grammar_proofread_engine.py)): LRU (max 128 entries) keyed by `doc_id`, locale, **SHA256 of the checked slice**, and **`(n_start, n_end)`** so identical substring text at different positions never aliases; `cache_get` still matches the stored text fingerprint. On cache miss, `doProofreading` starts/joins one in-flight worker per full cache key and waits up to `doc.grammar_proofreader_wait_timeout_ms` while pumping `processEventsToIdle()`; timeout falls back to cached-later behavior. Grammar LLM calls may pass `response_format={"type":"json_object"}` on the OpenAI-compatible HTTP path ([`LlmClient.make_chat_request`](plugin/modules/http/client.py)); native Google/Anthropic shims omit that field. The proofreader emits `grammar:status`; [`panel.py`](plugin/modules/chatbot/panel.py) listens and posts concise progress to the sidebar status field when chat is not busy. Grammar/chat LLM calls can share [`plugin/framework/llm_concurrency.py`](plugin/framework/llm_concurrency.py) `llm_request_lane()` to avoid overlap races. Startup does not call `setConfiguredServices`. Parity: `GRAMMAR_REGISTRY_LOCALE_TAGS` in [`grammar_proofread_engine.py`](plugin/modules/writer/grammar_proofread_engine.py) and [`plugin/tests/test_grammar_linguistic_xcu.py`](plugin/tests/test_grammar_linguistic_xcu.py).
