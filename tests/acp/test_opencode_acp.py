...
class TestOpenCodeBinaryDiscovery:
    """Test binary / identity hooks used by ACPBackend._find_binary()."""

    def test_binary_name_is_opencode(self):
        backend = OpenCodeBackend()
        assert (backend.get_binary_name()) == ("opencode")

    def test_display_name(self):
        backend = OpenCodeBackend()
        assert (backend.get_display_name()) == ("OpenCode (ACP)")

    def test_agent_name(self):
        backend = OpenCodeBackend()
        assert (backend.get_agent_name()) == ("opencode")


class TestOpenCodeBackendInit:
    """Test backend initialization."""

    def test_backend_id(self):
        backend = OpenCodeBackend()
        assert (backend.backend_id) == ("opencode")
        assert (backend.get_display_name()) == ("OpenCode (ACP)")
...
class TestAgentBackendDisplayLabel:
    ...
    def test_label_opencode(self):
        assert (_agent_backend_label(OpenCodeBackend(), "opencode")) == ("OpenCode (ACP)")
